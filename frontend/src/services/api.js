...
// ADD THIS METHOD inside analyticsAPI object
getDashboardQuickStats: async (userId) => {
  try {
    console.log("📊 Analytics API: Getting dashboard quick stats for user:", userId);
    
    const response = await api.get("/api/dashboard/quick-stats/", {
      params: { user_id: userId }
    });
    
    console.log("✅ Analytics API: Dashboard quick stats response:", response);
    return response;
  } catch (error) {
    console.error("❌ Analytics API: Failed to get dashboard quick stats:", error);
    throw new Error(
      error.response?.data?.error ||
        error.message ||
        "Failed to get dashboard quick stats"
    );
  }
},
  getParticipantDurationAnalytics: (params = {}) => {
...
export const recordingsAPI = {
  viewSummary: (id, email, userId) => {
    ...
    // ✅ FIXED: Changed from /api/videos/doc/ to /api/videos/document/
    return `${API_BASE_URL}/api/videos/document/${id}/summary?action=view&email=${encodeURIComponent(
      email
  ...
  downloadTranscript: async (id, email, userId, suggestedFileName = null) => {
    try {
      if (!id || !email || !userId) {
        throw new Error("Missing required parameters: id, email, or userId");
      }

      console.log("📥 [DOWNLOAD] Starting transcript download for recording:", id);

      // Use correct endpoint
      const url = `${API_BASE_URL}/api/videos/document/${id}/transcript?action=download&email=${encodeURIComponent(
        email
      )}&user_id=${encodeURIComponent(userId)}`;
      ...
      if (contentType.includes("vnd.openxmlformats-officedocument.wordprocessingml.document")) {
        ...
        console.log("✅ [DETECT] Method 1 (Content-Type): DOCX detected");
      } else if (contentType.includes("application/pdf")) {
        fileExtension = ".pdf";
        mimeType = "application/pdf";
        console.log("✅ [DETECT] Method 1 (Content-Type): PDF detected");
      }

      // =====================================================
      // METHOD 2: Check Content-Disposition filename
      // =====================================================
      if (!fileExtension) {
        const filenameMatch = contentDisposition.match(/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/);
        if (filenameMatch && filenameMatch[1]) {
          const headerFilename = filenameMatch[1].replace(/['"]/g, "").trim();
          console.log("📥 [DOWNLOAD] Filename from header:", headerFilename);
          
          if (headerFilename.toLowerCase().endsWith(".docx")) {
            fileExtension = ".docx";
            mimeType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
            console.log("✅ [DETECT] Method 2 (Content-Disposition): DOCX detected");
          } else if (headerFilename.toLowerCase().endsWith(".pdf")) {
            fileExtension = ".pdf";
            mimeType = "application/pdf";
            console.log("✅ [DETECT] Method 2 (Content-Disposition): PDF detected");
          }
        }
      }

      // =====================================================
      // METHOD 3: Check blob type
      // =====================================================
      if (!fileExtension && response.data.type) {
        const blobType = response.data.type.toLowerCase();
        console.log("📥 [DOWNLOAD] Checking blob type:", blobType);
        
        if (blobType.includes("officedocument") || blobType.includes("docx")) {
          fileExtension = ".docx";
          mimeType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
          console.log("✅ [DETECT] Method 3 (Blob type): DOCX detected");
        } else if (blobType.includes("pdf")) {
          fileExtension = ".pdf";
          mimeType = "application/pdf";
          console.log("✅ [DETECT] Method 3 (Blob type): PDF detected");
        }
      }

      // =====================================================
      // METHOD 4: Check magic bytes (file signature)
      // DOCX files start with: 50 4B 03 04 (PK..)
      // PDF files start with: 25 50 44 46 (%PDF)
      // =====================================================
      if (!fileExtension) {
        console.log("📥 [DOWNLOAD] Checking magic bytes...");
        try {
          ...
          console.log("📥 [DOWNLOAD] First 4 bytes (hex):", hexBytes);

          // DOCX/ZIP signature: 50 4B 03 04 (PK..)
          if (bytes[0] === 0x50 && bytes[1] === 0x4B && bytes[2] === 0x03 && bytes[3] === 0x04) {
            fileExtension = ".docx";
            mimeType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
            console.log("✅ [DETECT] Method 4 (Magic bytes): DOCX detected (PK signature)");
          }
          // PDF signature: 25 50 44 46 (%PDF)
          else if (bytes[0] === 0x25 && bytes[1] === 0x50 && bytes[2] === 0x44 && bytes[3] === 0x46) {
            fileExtension = ".pdf";
            mimeType = "application/pdf";
            console.log("✅ [DETECT] Method 4 (Magic bytes): PDF detected (%PDF signature)");
          }
        } catch (e) {
          console.warn("⚠️ [DOWNLOAD] Could not read magic bytes:", e);
        }
      }

      // =====================================================
      // FALLBACK: Default to PDF if nothing detected
      // =====================================================
      if (!fileExtension) {
        console.warn("⚠️ [DOWNLOAD] Could not detect file type, defaulting to PDF");
        fileExtension = ".docx";
        mimeType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
      }

      console.log("📥 [DOWNLOAD] Final detected extension:", fileExtension);

      // Build filename
      let fileName = suggestedFileName || `transcript_${id}`;
      
      // Remove any existing extension
      fileName = fileName.replace(/\.(pdf|docx|doc|mp4|webm)$/i, "");
      ...
      console.log("✅ [DOWNLOAD] Transcript download completed:", fileName);
      ...
      console.error("❌ [DOWNLOAD] Failed to download transcript:", error);
      throw new Error(
        error.response?.data?.Error || error.message || "Failed to download transcript"
      );
    }
  },


  downloadSummary: async (id, email, userId, suggestedFileName = null) => {
    try {
      if (!id || !email || !userId) {
        throw new Error("Missing required parameters: id, email, or userId");
      }

      console.log("📥 [DOWNLOAD] Starting summary download for recording:", id);

      // Use correct endpoint
      const url = `${API_BASE_URL}/api/videos/document/${id}/summary?action=download&email=${encodeURIComponent(
        email
      )}&user_id=${encodeURIComponent(userId)}`;
      ...
      // Build filename
      let fileName = suggestedFileName || `summary_${id}`;
      ...
      console.log("✅ [DOWNLOAD] Summary download completed:", fileName);
      ...
      console.error("❌ [DOWNLOAD] Failed to download summary:", error);
      throw new Error(
        error.response?.data?.Error || error.message || "Failed to download summary"
      );
    }
  },
  ...
  try {
    ...
    // =====================================================
    // DETECTION PRIORITY ORDER - FIXED
    // =====================================================
    let fileExtension = null;
    let mimeType = null;

    // METHOD 1: Check magic bytes FIRST (most reliable)
    console.log("📥 [DOWNLOAD] Checking magic bytes...");
    try {
      const arrayBuffer = await response.data.slice(0, 4).arrayBuffer();
      const bytes = new Uint8Array(arrayBuffer);
      ...
      if (bytes[0] === 0x50 && bytes[1] === 0x4B && bytes[2] === 0x03 && bytes[3] === 0x04) {
        fileExtension = ".docx";
        mimeType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
        console.log("✅ [DETECT] Magic bytes: DOCX detected (PK signature)");
      }
      // PDF signature: 25 50 44 46 (%PDF)
      else if (bytes[0] === 0x25 && bytes[1] === 0x50 && bytes[2] === 0x44 && bytes[3] === 0x46) {
        fileExtension = ".pdf";
        mimeType = "application/pdf";
        console.log("✅ [DETECT] Magic bytes: PDF detected (%PDF signature)");
      }
    } catch (e) {
      console.warn("⚠️ [DOWNLOAD] Could not read magic bytes:", e);
    }

    // METHOD 2: Check Content-Type header (if magic bytes failed)
    if (!fileExtension) {
      if (contentType.includes("vnd.openxmlformats-officedocument.wordprocessingml.document") ||
        ...
        fileExtension = ".docx";
        mimeType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
        console.log("✅ [DETECT] Content-Type: DOCX detected");
      } else if (contentType.includes("application/pdf")) {
        fileExtension = ".pdf";
        mimeType = "application/pdf";
        console.log("✅ [DETECT] Content-Type: PDF detected");
      }
    }

    // METHOD 3: Check Content-Disposition filename
    if (!fileExtension) {
      const filenameMatch = contentDisposition.match(/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/);
      if (filenameMatch && filenameMatch[1]) {
        const headerFilename = filenameMatch[1].replace(/['"]/g, "").trim();
        ...
        if (headerFilename.toLowerCase().endsWith(".docx")) {
          fileExtension = ".docx";
          mimeType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
          console.log("✅ [DETECT] Content-Disposition: DOCX detected");
        } else if (headerFilename.toLowerCase().endsWith(".pdf")) {
          fileExtension = ".pdf";
          mimeType = "application/pdf";
          console.log("✅ [DETECT] Content-Disposition: PDF detected");
        }
      }
    }

    // =====================================================
    // FALLBACK: Default to DOCX (since your system uses DOCX)
    // =====================================================
    if (!fileExtension) {
      console.warn("⚠️ [DOWNLOAD] Could not detect file type, defaulting to DOCX");
      fileExtension = ".docx";  // ← CHANGED FROM .pdf to .docx
      mimeType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
    }

    console.log("📥 [DOWNLOAD] Final detected extension:", fileExtension);

    // Build filename - REMOVE any wrong extension first
    let fileName = suggestedFileName || `transcript_${id}`;
    fileName = fileName.replace(/\.(pdf|docx|doc|mp4|webm)$/i, "");
    fileName = `${fileName}${fileExtension}`;
  ...
  downloadSummary: async (id, email, userId, suggestedFileName = null) => {
    try {
      ...
      console.log("📥 Downloading summary for recording:", id);

      // ✅ FIXED: Use correct endpoint /api/videos/document/
      const url = `${API_BASE_URL}/api/videos/document/${id}/summary?action=download&email=${encodeURIComponent(
        email
      )}&user_id=${encodeURIComponent(userId)}`;
      ...
      // ✅ CRITICAL FIX: Detect file type from Content-Type header
      const contentType = response.headers["content-type"] || "";
      const contentDisposition = response.headers["content-disposition"] || "";
      ...
      if (
        ...
        fileExtension = ".docx";
        mimeType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
        console.log("📄 Detected DOCX format from Content-Type");
      } else if (contentType.includes("application/pdf")) {
        fileExtension = ".pdf";
        mimeType = "application/pdf";
        console.log("📄 Detected PDF format from Content-Type");
      }
      ...
      if (!fileName) {
        fileName = `summary_${id}`;
      }

      // ✅ CRITICAL: Ensure correct extension - remove any wrong extension first
      fileName = fileName.replace(/\.(pdf|docx|doc)$/i, "");
      fileName = `${fileName}${fileExtension}`;
      ...
      console.log("✅ Summary download initiated:", fileName);
  ...
  getDocument: async (id, docType, email, userId, action = "download") => {
    try {
      ...
      if (!["transcript", "summary"].includes(docType)) {
        throw new Error("Invalid document type. Use 'transcript' or 'summary'");
      }

      // ✅ FIXED: Use correct endpoint /api/videos/document/
      const url = `${API_BASE_URL}/api/videos/document/${id}/${docType}?action=${action}&email=${encodeURIComponent(
        email
      )}&user_id=${encodeURIComponent(userId)}`;

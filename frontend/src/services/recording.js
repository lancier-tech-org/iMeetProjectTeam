...
export const recordingsAPI = {
  ...
  getThumbnailUrl: (id, email, userId) => {
    ...
    return `${API_BASE_URL}/api/videos/${id}/thumbnail?email=${encodeURIComponent(email)}&user_id=${encodeURIComponent(userId)}`;
  },

  // FIXED: Check if subtitles are available - with proper auth and correct response path
  checkSubtitlesAvailable: async (recordingId) => {
    try {
      ...
      if (!userId || !userEmail) {
        console.error('❌ API: Missing authentication data for subtitle check');
        return false;
      }
      
      // Get the video document with auth
  ...
  // NEW: Get available subtitle languages from recording data
  getAvailableSubtitleLanguages: async (recordingId) => {
    try {
      console.log('🌐 API: Getting available subtitle languages for:', recordingId);
      ...
      const availableLanguages = Object.keys(subtitles)
        .filter(lang => subtitles[lang] && subtitles[lang].trim() !== '')
        .map(lang => ({
          code: lang,
      ...
      console.log('✅ API: Found', availableLanguages.length, 'subtitle languages');
      return availableLanguages;
    } catch (error) {
      console.error('❌ API: Failed to get available subtitle languages:', error);
      return [];
    }
  },
  ...
  try {
    ...
    // CRITICAL FIX: Django endpoint expects MongoDB _id, NOT meeting_id
    // So we use recordingId directly without conversion
    console.log('📥 API: Fetching subtitles with ID:', recordingId);

// Enhanced RecordingsPage.jsx - Teal-Blue Theme Version with Transcript Viewer
// ALL STRUCTURE PRESERVED - Added Transcript Viewer Modal
...
const RecordingsPage = () => {
  ...
  // NEW: Transcript Viewer State
  const [transcriptViewerOpen, setTranscriptViewerOpen] = useState(false);
  const [transcriptViewerData, setTranscriptViewerData] = useState(null);
  const [transcriptLoading, setTranscriptLoading] = useState(false);

  // NEW: Summary Viewer State
  const [summaryViewerOpen, setSummaryViewerOpen] = useState(false);
  const [summaryViewerData, setSummaryViewerData] = useState(null);
  ...
  // Auto-play recording from notification click
  useEffect(() => {
  ...
  // =====================================================
  // NEW: View Transcript Function - Opens Modal with Content
  // =====================================================
  const handleViewTranscript = async (recording) => {
    ...
    try {
      ...
      // Fetch transcript content as JSON from your backend
      const url = `${API_BASE_URL}/api/videos/transcript-content/${recordingId}?email=${encodeURIComponent(userEmail)}&user_id=${encodeURIComponent(userId)}`;
  ...
  const handleCopyTranscript = async () => {
    if (transcriptViewerData?.full_text) {
      try {
        await navigator.clipboard.writeText(transcriptViewerData.full_text);
        showNotification("Copied", "Transcript copied to clipboard!", "success");
      } catch (err) {
        showNotification("Error", "Failed to copy transcript", "error");
      }
    }
  };

  // =====================================================
  // Download Transcript Function (unchanged - downloads PDF)
  // =====================================================
  const handleDownloadTranscript = async (recording) => {
    ...
    try {
      // ✅ FIXED: Don't pass a filename with extension
      // Let the API auto-detect DOCX vs PDF from Content-Type header
      const result = await downloadTranscript(
        recording.id,
        currentUser.email,
        currentUser.id,
        null  // ✅ Pass null - let API determine filename and extension
      );

      showNotification(
        "Download Started",
        `Transcript downloaded: ${result?.fileName || 'transcript'}`,
        "success"
      );
    } catch (err) {
  ...
  // =====================================================
  // NEW: View Summary Function - Opens Modal with Content
  // =====================================================
  const handleViewSummary = async (recording) => {
    ...
    try {
      ...
      setSummaryViewerData(null);

      const recordingId = recording.id || recording._id;
      const userEmail = currentUser.email || localStorage.getItem('user_email') || '';
      const userId = currentUser.id || localStorage.getItem('user_id') || '';

      // Fetch summary content as JSON from your backend
      // You'll need to create this endpoint similar to transcript-content
      const url = `${API_BASE_URL}/api/videos/summary-content/${recordingId}?email=${encodeURIComponent(userEmail)}&user_id=${encodeURIComponent(userId)}`;
  ...
  // Close Summary Viewer
  const handleCloseSummaryViewer = () => {
    setSummaryViewerOpen(false);
    setSummaryViewerData(null);
    setSummaryLoading(false);
  };
  ...
  const handleCopySummary = async () => {
    if (summaryViewerData?.full_text) {
      try {
        await navigator.clipboard.writeText(summaryViewerData.full_text);
        showNotification("Copied", "Summary copied to clipboard!", "success");
      } catch (err) {
        showNotification("Error", "Failed to copy summary", "error");
      }
    }
  };

  // =====================================================
  // Download Summary Function (unchanged - downloads PDF)
  // =====================================================
  const handleDownloadSummary = async (recording) => {
    ...
    try {
      // ✅ FIXED: Don't pass a filename with extension
      // Let the API auto-detect DOCX vs PDF from Content-Type header
      const result = await downloadSummary(
        recording.id,
        currentUser.email,
        currentUser.id,
        null  // ✅ Pass null - let API determine filename and extension
      ...
      showNotification(
        "Download Started",
        `Summary downloaded: ${result?.fileName || 'summary'}`,
        "success"
      );
    } catch (err) {
      ...
      showNotification(
        "Download Failed",
        "Failed to download summary: " + err.message,
        "error"
      );
    }
  };
    ...
    <DashboardLayout>
      ...
      >
        ...
        <Dialog
          ...
          onClose={handleCloseTranscriptViewer}
          maxWidth="md"
          fullWidth
          PaperProps={{
            sx: {
        ...
        >
            ...
            <Stack direction="row" alignItems="center" spacing={2}>
              ...
              <Box>
                ...
                <Typography variant="caption" sx={{ opacity: 0.9 }}>
                  {transcriptViewerData?.meeting_name || transcriptViewerData?.recording?.meeting_name || 'Meeting Recording'}
                </Typography>
              </Box>
            </Stack>
            <IconButton onClick={handleCloseTranscriptViewer} sx={{ color: 'white' }}>
              <CloseIcon />
            </IconButton>
          </DialogTitle>
          ...
          <DialogContent sx={{ p: 0 }}>
            ...
            {/* Transcript Content */}
            {transcriptViewerData && !transcriptLoading && (
              <Box>
                {/* Metadata Bar */}
                <Paper
                  elevation={0}
                ...
                >
                  ...
                  >
                    <Stack direction="row" spacing={2} flexWrap="wrap" useFlexGap>
                      {transcriptViewerData.date && (
                        <Chip
                          icon={<TimeIcon />}
                          label={format(new Date(transcriptViewerData.date), "MMM dd, yyyy • HH:mm")}
                          size="small"
                          variant="outlined"
                          sx={{ borderColor: themeColors.teal, color: themeColors.teal }}
                        />
                      )}
                      {transcriptViewerData.word_count && (
                        <Chip
                          label={`${transcriptViewerData.word_count.toLocaleString()} words`}
                          size="small"
                          variant="outlined"
                          sx={{ borderColor: themeColors.blue, color: themeColors.blue }}
                        />
                      )}
                      {transcriptViewerData.meeting_id && (
                        <Chip
                          label={`Meeting: ${transcriptViewerData.meeting_id}`}
                          size="small"
                          variant="outlined"
                        />
                      )}
                ...
                >
                  {transcriptViewerData.paragraphs && transcriptViewerData.paragraphs.length > 0 ? (
                    transcriptViewerData.paragraphs.map((para, index) => (
                  ...
                  ) : transcriptViewerData.full_text ? (
                    <Typography
                      variant="body1"
                      sx={{
                        lineHeight: 1.8,
            ...
            {!transcriptViewerData && !transcriptLoading && (
              ...
              >
                <TranscriptIcon sx={{ fontSize: 64, color: 'text.disabled' }} />
          ...
          <DialogActions sx={{ p: 2, justifyContent: 'space-between' }}>
            <Button onClick={handleCloseTranscriptViewer} color="inherit">
              Close
            </Button>
            <Stack direction="row" spacing={1}>
              <Button
                ...
                disabled={!transcriptViewerData?.full_text}
                sx={{
                  borderColor: themeColors.blue,
                  color: themeColors.blue,
                  '&:hover': { borderColor: themeColors.teal, bgcolor: alpha(themeColors.teal, 0.05) },
                }}
              >
                Copy Text
              </Button>
              <Button
                ...
                onClick={() => {
                  if (transcriptViewerData?.recording) {
                    handleDownloadTranscript(transcriptViewerData.recording);
                  }
                }}
                disabled={!transcriptViewerData?.recording}
                sx={{
                  bgcolor: themeColors.teal,
        ...
        <Dialog
          ...
          onClose={handleCloseSummaryViewer}
          maxWidth="md"
          fullWidth
          PaperProps={{
            sx: {
        ...
        >
          ...
          <DialogTitle
            sx={{
              background: `linear-gradient(135deg, ${themeColors.blue} 0%, ${themeColors.purple} 100%)`,
              color: 'white',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'space-between',
          ...
          >
            <Stack direction="row" alignItems="center" spacing={2}>
              ...
              <Box>
                ...
                <Typography variant="caption" sx={{ opacity: 0.9 }}>
                  {summaryViewerData?.meeting_name || summaryViewerData?.recording?.meeting_name || 'Meeting Recording'}
                </Typography>
              </Box>
            </Stack>
            <IconButton onClick={handleCloseSummaryViewer} sx={{ color: 'white' }}>
              <CloseIcon />
            </IconButton>
          </DialogTitle>
          ...
          <DialogContent sx={{ p: 0 }}>
            ...
            {/* Summary Content */}
            {summaryViewerData && !summaryLoading && (
              <Box>
                {/* Metadata Bar */}
                <Paper
                  elevation={0}
                  sx={{
                    ...
                    border: `1px solid ${alpha(themeColors.purple, 0.1)}`,
                  }}
                >
                  <Stack
                    direction={{ xs: 'column', sm: 'row' }}
                  ...
                  >
                    <Stack direction="row" spacing={2} flexWrap="wrap" useFlexGap>
                      {summaryViewerData.date && (
                        <Chip
                          icon={<TimeIcon />}
                          label={format(new Date(summaryViewerData.date), "MMM dd, yyyy • HH:mm")}
                          size="small"
                          variant="outlined"
                          sx={{ borderColor: themeColors.blue, color: themeColors.blue }}
                        />
                      )}
                      {summaryViewerData.word_count && (
                        <Chip
                          label={`${summaryViewerData.word_count.toLocaleString()} words`}
                          size="small"
                          variant="outlined"
                          sx={{ borderColor: themeColors.purple, color: themeColors.purple }}
                        />
                      )}
                    </Stack>
                  </Stack>
                ...
                >
                  {summaryViewerData.paragraphs && summaryViewerData.paragraphs.length > 0 ? (
                    summaryViewerData.paragraphs.map((para, index) => (
                      <Box key={index} sx={{ mb: 2 }}>
                        {para.style === 'heading' ? (
                          <Typography
                            variant="h6"
                    ...
                    <Alert severity="info">No summary content available</Alert>
                  )}
                </Box>
              </Box>
            )}
            ...
            {!summaryViewerData && !summaryLoading && (
              ...
              >
                <SummaryIcon sx={{ fontSize: 64, color: 'text.disabled' }} />
          ...
          <Divider />
          <DialogActions sx={{ p: 2, justifyContent: 'space-between' }}>
            <Button onClick={handleCloseSummaryViewer} color="inherit">
              Close
            </Button>
            <Stack direction="row" spacing={1}>
              <Button
                ...
                disabled={!summaryViewerData?.full_text}
                sx={{
                  borderColor: themeColors.purple,
                  color: themeColors.purple,
                  '&:hover': { borderColor: themeColors.blue, bgcolor: alpha(themeColors.blue, 0.05) },
                }}
              >
                Copy Text
              </Button>
              <Button
                ...
                startIcon={<DownloadIcon />}
                onClick={() => {
                  if (summaryViewerData?.recording) {
                    handleDownloadSummary(summaryViewerData.recording);
                  }
                }}
                disabled={!summaryViewerData?.recording}
                sx={{
                  bgcolor: themeColors.blue,
                  '&:hover': { bgcolor: themeColors.purple },
                }}
              >
                Download PDF
              </Button>

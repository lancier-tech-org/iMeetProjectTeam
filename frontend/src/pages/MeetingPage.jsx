...
// Professional Styled Components matching the UI design
...
const ErrorContainer = styled(Box)(({ theme }) => ({
  ...
  alignItems: 'center',
  justifyContent: 'center',
  background: "linear-gradient(135deg, #5689ffff 0%, #79a4eaff 50%, #5a86edff 100%)",
  color: 'white',
  textAlign: 'center',
  padding: theme.spacing(4),
  position: 'relative',
...
const MeetingPage = () => {
  ...
  const [isLockedInMeeting, setIsLockedInMeeting] = useState(false); // ✅ NEW: Prevents going back to WaitingRoom
  ...
  } = useWebRTC(actualMeetingId, user?.id, isHost);
  ...
  useEffect(() => {
    ...
    if (isLiveKitFullyConnected && hasTriedJoin && !isJoining && !livekitConnecting) {
      ...
      if (showWaitingRoom) {
        ...
        setIsLockedInMeeting(true); // ✅ NEW: Lock user in MeetingRoom
  ...
  const handleLeaveMeeting = useCallback(async () => {
    ...
    try {
      ...
      setIsLockedInMeeting(false); // ✅ NEW: Reset lock on leave
      setHasTriedJoin(false);
  ...
  const shouldShowWaitingRoom = !isLockedInMeeting && (
    isJoining ||
    livekitConnecting ||
    !livekitConnected ||
    !hasTriedJoin ||

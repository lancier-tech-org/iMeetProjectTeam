...
const [fetchedMeetingDetails, setFetchedMeetingDetails] = useState(null);
  ...
  // Create instant meeting when meetingId is 'instant'
  useEffect(() => {
    if (meetingId === 'instant' && !actualMeeting && !isCreatingMeeting && !meetingEnded) {
      createInstantMeeting();
    }
  }, [meetingId, actualMeeting, isCreatingMeeting, meetingEnded]);
  ...
  const handleLeaveMeeting = useCallback(async () => {
    ...
    try {
      // ✅ FIX: Navigate FIRST, before any state resets
      // This prevents useEffect from seeing actualMeeting=null while still on /meeting/instant
      navigate('/dashboard');
      ...
      // Step 4: Reset all states (safe now - component is unmounting)
      setWebrtcInitialized(false);
      setShowWaitingRoom(true);
      setHasTriedJoin(false);
      setRealMeetingIdState(null);
  ...
  // Create instant meeting via API
  const createInstantMeeting = async () => {
    // ✅ FIX: Double-check meetingEnded flag to prevent duplicate creation
    if (meetingEnded) {
      console.log('⛔ createInstantMeeting blocked - meeting already ended');
      return;
    }
    setIsCreatingMeeting(true);
    try {

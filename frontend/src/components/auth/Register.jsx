...
const CustomStepIcon = styled("div")(({ theme, ownerState }) => ({
  backgroundColor:
    ownerState.completed || ownerState.active ? "#2196F3" : "#E0E0E0",
  color: "#FFFFFF",
  width: 36,
  height: 36,
  display: "flex",
...
const Register = () => {
  ...
  React.useEffect(() => {
    if (formData.phone_number) {
      const validation = validatePhoneRealTime(
  ...
  // Phone number change handler with real-time validation
  ...
  const handleCountryCodeChange = (e) => {
    ...
    if (formData.phone_number) {
      const validation = validatePhoneRealTime(formData.phone_number, newCode);
      setPhoneValidation(validation);
    }
  ...
  const handleEmailBlur = () => {
    ...
    if (
      ...
      setEmailError("Invalid email format. Please use format: name@gmail.com");
      return;
    }
    ...
    // Valid domains (same as your backend)
    const validDomains = [
      "gmail.com",
      "googlemail.com",
      "outlook.com",
    ...
    if (domain.includes("gmail")) {
      setEmailError(
        `Invalid email domain '${domain}'. Did you mean gmail.com?`
      );
    } else if (domain.includes("yahoo")) {
      setEmailError(
        `Invalid email domain '${domain}'. Did you mean yahoo.com?`
      );
    } else if (domain.includes("hotmail")) {
      setEmailError(
        `Invalid email domain '${domain}'. Did you mean hotmail.com?`
      );
    } else if (domain.includes("outlook")) {
      setEmailError(
        `Invalid email domain '${domain}'. Did you mean outlook.com?`
      );
    } else {
      setEmailError(
        `Email domain '${domain}' is not supported. Please use Gmail, Yahoo, Outlook, or other major providers.`
      );
    }
  };
  const validateStep = (step) => {
    ...
    switch (step) {
      case 0:
        ...
        else if (emailError) newErrors.email = emailError; // Add domain validation error
        if (!(profilePhoto || profilePhotoRef.current))
      ...
      case 1:
        ...
        break;
      case 2:
        // DO NOT validate here - validation happens ONLY in handleSubmit
        break;
      default:
        break;
    }
  ...
  const handleNext = () => {
    if (validateStep(activeStep)) {
      setErrors({});
      // Use callback to ensure errors are cleared before step changes
      setTimeout(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate password fields directly here
    const newErrors = {};
    ...
    if (!photo) {
      setApiError("Please capture a profile photo");
      setActiveStep(0);
      return;
    }
    if (!photo.startsWith("data:image")) {
      setApiError("Invalid photo format");
      setActiveStep(0);
      return;
    }
  ...
  const renderStepContent = (step) => {
    switch (step) {
        ...
        return (
          <Stack spacing={{ xs: 2, sm: 3 }}>
                ...
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">
                      <Person
                        sx={{ color: "#9E9E9E", fontSize: { xs: 18, sm: 20 } }}
                      />
                    </InputAdornment>
                  ),
                }}
            ...
            <Box sx={{ textAlign: "center", py: { xs: 1.5, sm: 2 } }}>
              <InputLabel sx={{ textAlign: "center", mb: { xs: 1.5, sm: 2 } }}>
                Profile Photo *
              </InputLabel>
              <Box
                ...
                sx={{
                  ...
                  borderRadius: "50%",
                  border: `2px dashed ${
                    errors.profilePhoto ? "#F44336" : "#E0E0E0"
                  }`,
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
              ...
              >
                {profilePhoto || profilePhotoRef.current ? (
                  ...
                  <Stack alignItems="center" spacing={0.5}>
                    <CameraAlt
                      sx={{ fontSize: { xs: 28, sm: 32 }, color: "#9E9E9E" }}
                    />
                    <Typography
                      variant="caption"
                      sx={{
          ...
          <Stack spacing={{ xs: 2, sm: 3 }}>
            <Box>
              <InputLabel>
                ...
                >
                  (
                  {PHONE_VALIDATION_RULES[formData.country_code]?.length?.[0] ||
                    10}{" "}
                  digits, starts with{" "}
                  {PHONE_VALIDATION_RULES[formData.country_code]?.startsWith ||
                    "any"}
                  )
                </Typography>
              </InputLabel>
                ...
                <Grid item xs={8} sm={8.5}>
                  <StyledTextField
                    ...
                    helperText={
                      ...
                      `Example: ${
                        PHONE_VALIDATION_RULES[formData.country_code]
                          ?.example || ""
                      }`
                    }
                    FormHelperTextProps={{
                      sx: {
                        color: errors.phone_number
                          ? "#F44336"
                          : phoneValidation.status === "error"
                          ? "#F44336"
                    ...
                    InputProps={{
                      ...
                      endAdornment: formData.phone_number && (
                        <InputAdornment position="end">
                          {phoneValidation.status === "valid" && (
                            <CheckCircle
                          ...
                          {phoneValidation.status === "error" && (
                            <span style={{ color: "#F44336", fontSize: 18 }}>
                              ⚠
                            </span>
                          )}
                        </InputAdornment>
              ...
              <FormControl fullWidth error={!!errors.languages}>
                    ...
                    selected.length === 0 ? (
                      ...
                      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                        ...
                        {selected.length > (isMobile ? 2 : 3) && (
                          <Chip
                            label={`+${selected.length - (isMobile ? 2 : 3)}`}
                            size="small"
                            sx={{
                              backgroundColor: "#E0E0E0",
                              fontSize: { xs: "0.7rem", sm: "0.75rem" },
                ...
                >
                  {languages.map((lang) => (
                    ...
                    >
                      <Checkbox
                        checked={formData.languages.indexOf(lang) > -1}
                        sx={{
                          color: "#E0E0E0",
      ...
      case 2:
        // Clear any lingering errors when this step first renders
        if (
          ...
          // Do nothing - errors are from previous steps
        }
        return (
          <Stack spacing={{ xs: 2, sm: 3 }}>
            <Box>
              ...
              <StyledTextField
                ...
                FormHelperTextProps={{
                  sx: { color: errors.password ? "#d32f2f" : "#666666" },
                }}
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end">
            ...
            >
              <FormControlLabel
                ...
                label={
                  ...
                  >
                    I agree to the{" "}
                    <Link href="#" sx={{ color: "#2196F3", fontWeight: 600 }}>
                      Terms of Service
                    </Link>{" "}
                    and{" "}
                    <Link href="#" sx={{ color: "#2196F3", fontWeight: 600 }}>
        ...
        >
          ...
          >
            ...
            <Box component="form" onSubmit={handleSubmit}>
              <Box
                sx={{ minHeight: { xs: 260, sm: 280 }, mb: { xs: 2, sm: 3 } }}
              >
              ...
              >
                ...
                {activeStep === steps.length - 1 ? (
                  ...
                  <Button
                    variant="contained"
                    endIcon={
                      <ArrowForward sx={{ fontSize: { xs: 18, sm: 20 } }} />
                    }
                    onClick={handleNext}
                    fullWidth={isMobile}
                    sx={{
        ...
        >
          <Box
            sx={{
              ...
              right: 0,
              bottom: 0,
              backgroundImage: `url('https://images.unsplash.com/photo-1560250097-0b93528c311a?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80')`,
              ...
              "&::before": {
                ...
                bottom: 0,
                background:
                  "linear-gradient(135deg, rgba(33, 150, 243, 0.85) 0%, rgba(61, 180, 172, 0.9) 100%)",
              },
            }}
          />
          <Box
          ...
          >
            ...
            >
              <VideoCall
                sx={{ fontSize: { md: 26, lg: 32 }, color: "#FFFFFF" }}
              />
            </Box>
            <Typography
              variant="h3"
            ...
            >
              ...
              with advanced collaboration features trusted by millions of

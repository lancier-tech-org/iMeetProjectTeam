...
const MeetingRoom = memo(function MeetingRoom({
  ...
  realMeetingId = null,
  participantId = null,
  remoteParticipants: propRemoteParticipants = new Map(),
  room: propRoom = null,
  localParticipant: propLocalParticipant = null,
}) {
  // ==========================================================================
  // LIVEKIT HOOKS
  // ==========================================================================
  ...
  // ✅ CRITICAL FIX: Use props from MeetingPage if available (shared connection)
  const effectiveRoom = propRoom || room;
  const effectiveLocalParticipant = propLocalParticipant || localParticipant;
        ...
        if (response.success) {
          const processedParticipants = response.participants.map(
            (participant) => ({
              ...
              // ✅ ADD: Email fields for participant display
          ...
          const activeParticipants = processedParticipants.filter((p) => {
            ...
            if (p.Status === "live" || p.Status === "connecting") {
              return true;
            }
            if (p.Leave_Time && !p.Is_Currently_Active && !p.LiveKit_Connected) {
              return false;
            }
            return true;
          });
  ...
  const allParticipants = useMemo(() => {
    ...
    if (currentUser) {
      ...
      participantMap.set(localKey, {
        ...
        // ✅ ADD: Email from existing participant or DB lookup
    ...
    if (remoteParticipants?.size > 0) {
      remoteParticipants.forEach((participant) => {
        ...
        // FIX: If userId is not a number, find matching participant from database
        const isNumericUserId = !isNaN(parseInt(userId));
        if (!isNumericUserId) {
          const matchingParticipant = liveParticipants.find((p) => {
            ...
            return pName === identityLower || pName.includes(identityLower) || identityLower.includes(pName.split(" ")[0]?.toLowerCase() || "");
          });
          
          if (matchingParticipant) {
            userId = (matchingParticipant.User_ID || matchingParticipant.user_id || matchingParticipant.ID)?.toString();
            console.log("Found userId " + userId + " for identity " + participant.identity);
          } else {
            console.warn("Could not find matching participant for identity: " + participant.identity);
            return; // Skip this participant - no valid userId
          }
        }
...
console.log("📹 Remote participant name resolved:", {
  ...
  hadExistingParticipant: !!existingParticipant,
        ...
        participantMap.set(userKey, {
          ...
          name: displayName,
          displayName: displayName,
          // ✅ ADD: Email from existing participant data
          email: existingParticipant?.email || existingParticipant?.Email || "",
          Email: existingParticipant?.Email || existingParticipant?.email || "",
          isLocal: false,

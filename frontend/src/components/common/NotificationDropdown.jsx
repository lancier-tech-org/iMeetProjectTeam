...
const NotificationDropdown = ({ open, onClose, anchorEl, filterType = 'all' }) => {
  ...
  // ✅ UPDATED: handleNotificationClick with correct navigation logic
const handleNotificationClick = async (notification) => {
  ...
  switch (notificationType) {
    ...
    // ✅ Schedule Meeting notifications - Navigate to schedule page
    case 'meeting_scheduled':
    case 'meeting_invitation':
    case 'scheduled_meeting_invitation':
    case 'scheduled_meeting_created':
    case 'schedule_meeting_reminder':
      console.log('📅 Navigating to schedule page...');
      onClose();
      if (notification.meeting_id) {
        navigate(`/schedule?meeting_id=${notification.meeting_id}`);
      } else {
        navigate('/schedule');
      }
      break;

    // ✅ Calendar Meeting notifications - Navigate to calendar page
    case 'calendar_meeting':
    case 'calendar_meeting_invitation':
    case 'calendar_meeting_created':
    case 'calendar_meeting_reminder':
      ...
      onClose();
      if (notification.meeting_id) {
        navigate(`/calendar?meeting_id=${notification.meeting_id}`);
      } else {
        navigate('/calendar');
      }
      break;

    // ✅ Meeting reminder/started - Navigate to meeting room or open URL
    case 'meeting_reminder':
    case 'meeting_started':
      ...
      } else if (notification.meeting_url) {
        if (notification.meeting_url.startsWith('http')) {
    ...
    // ✅ Meeting ended - Navigate to analytics
    case 'meeting_ended':
      console.log('📊 Navigating to meeting analytics...');
      onClose();
      if (notification.meeting_id) {
        navigate(`/analytics?meeting_id=${notification.meeting_id}`);
      } else {
        navigate('/analytics');
      }
      break;

    // ✅ Meeting created (generic) - Check meeting type from notification data
    case 'meeting_created':
      console.log('📋 Meeting created notification...');
      onClose();
      // Try to determine if it's calendar or schedule based on notification data
      const meetingType = notification.meeting_type || notification.data?.meeting_type;
      if (meetingType === 'CalendarMeeting') {
        navigate(notification.meeting_id ? `/calendar?meeting_id=${notification.meeting_id}` : '/calendar');
      } else {
        navigate(notification.meeting_id ? `/schedule?meeting_id=${notification.meeting_id}` : '/schedule');
      }
      break;
    ...
    // ✅ Default behavior
    default:
      console.log('Default notification click handler for type:', notificationType);
      onClose();
      
      // Try to navigate based on available data
      if (notification.meeting_url) {
        ...
        // Default to schedule page if we have a meeting_id
        navigate(`/schedule?meeting_id=${notification.meeting_id}`);
      }
      break;
  }

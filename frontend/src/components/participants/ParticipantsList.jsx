// src/components/participants/ParticipantsList.jsx - COMPLETE FIXED VERSION
...
const ParticipantsList = ({ 
  ...
  // ============================================================================
  // CRITICAL FIX: Persistent cache to preserve participant identity data
  // ============================================================================
  const nameResolutionCache = useRef(new Map());
  const participantDataCache = useRef(new Map());

  const safeParticipants = Array.isArray(participants) ? participants : [];

  // ============================================================================
  // CRITICAL FIX: Cache participant data when it's first loaded or updated
  // This prevents name/email from being overwritten by incomplete updates
  // ============================================================================
  useEffect(() => {
    if (Array.isArray(participants)) {
      participants.forEach(participant => {
        ...
        const existingCache = participantDataCache.current.get(participantId) || {};
        
        // Build complete participant data, preferring non-empty values
        const completeData = {
          // Preserve all identity fields
          id: participant.id || existingCache.id,
          user_id: participant.user_id || existingCache.user_id,
          User_ID: participant.User_ID || existingCache.User_ID,
          ID: participant.ID || existingCache.ID,
          
          // CRITICAL: Preserve name fields - never overwrite with empty/placeholder values
          full_name: (participant.full_name && !participant.full_name.startsWith('User')) 
            ? participant.full_name 
            : existingCache.full_name,
          Full_Name: (participant.Full_Name && !participant.Full_Name.startsWith('User'))
            ? participant.Full_Name
            : existingCache.Full_Name,
          name: (participant.name && !participant.name.startsWith('User'))
            ? participant.name
            : existingCache.name,
          displayName: (participant.displayName && !participant.displayName.startsWith('User'))
            ? participant.displayName
            : existingCache.displayName,
          user_name: (participant.user_name && !participant.user_name.startsWith('User'))
            ? participant.user_name
            : existingCache.user_name,
          
          // CRITICAL: Preserve email - never overwrite with 'No email'
          email: (participant.email && participant.email !== 'No email')
            ? participant.email
            : existingCache.email,
          Email: (participant.Email && participant.Email !== 'No email')
            ? participant.Email
          ...
          profile_picture: participant.profile_picture || existingCache.profile_picture,
          avatar: participant.avatar || existingCache.avatar,
          role: participant.role || existingCache.role,
          Role: participant.Role || existingCache.Role,
          isHost: participant.isHost !== undefined ? participant.isHost : existingCache.isHost,
          
          // Update dynamic status fields (these CAN be overwritten)
          ...
          LiveKit_Connected: participant.LiveKit_Connected,
          Join_Time: participant.Join_Time || existingCache.Join_Time,
        ...
        // Update cache
        participantDataCache.current.set(participantId, completeData);
      });
    }
  }, [participants]);

  // ============================================================================
  // ENHANCED: getParticipantDisplayName with cache fallback
  // ============================================================================
  const getParticipantDisplayName = useCallback((participant) => {
    ...
    const cachedData = participantDataCache.current.get(participantId);
    
    // Check if we already have a resolved name in the name cache
    if (nameResolutionCache.current.has(participantId)) {
      const cachedName = nameResolutionCache.current.get(participantId);
      if (cachedName && cachedName !== 'Unknown User' && !cachedName.startsWith('User')) {
        return cachedName;
      }
    }
    
    // Combine current participant data with cached data
    const mergedData = {
      ...cachedData,
      ...participant,
      // CRITICAL: Prefer cached name fields if current ones are placeholders
      full_name: (participant.full_name && !participant.full_name.startsWith('User'))
        ? participant.full_name
    ...
    for (const nameField of nameFields) {
      if (nameField && 
        ...
        nameResolutionCache.current.set(participantId, cleanName);
        return cleanName;
      }
    }
  ...
  // ============================================================================
  // NEW: Get participant email with cache fallback
  // ============================================================================
  const getParticipantEmail = useCallback((participant) => {
    if (!participant) return 'No email';
    ...
    const email = (participant.email && participant.email !== 'No email')
      ? participant.email
      : (participant.Email && participant.Email !== 'No email')
      ? participant.Email
      : (cachedData?.email && cachedData.email !== 'No email')
      ? cachedData.email
      : (cachedData?.Email && cachedData.Email !== 'No email')
      ? cachedData.Email
      : 'No email';
  ...
  }, [safeParticipants, searchTerm, removedParticipants, getParticipantDisplayName, getParticipantEmail, getParticipantStatus]);

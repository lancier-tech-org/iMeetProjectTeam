...
const RecordingPlayer = ({ recordingData = {} }) => {
  ...
  const [duration, setDuration] = useState(Number(recordingData.duration) || 0);
  ...
  useEffect(() => {
    const recordingId = recordingData.meeting_id || recordingData.id || recordingData._id;
    if (recordingId) {
  ...
  try {
    ...
    // Step 1: Get recording data with proper auth
    ...
    if (!userId || !userEmail) {
      console.error('❌ DEBUG: Missing authentication data');
    ...
    try {
      // Call API with proper query parameters
    ...
    // Step 2: Extract subtitles directly from response
    console.log('📋 DEBUG: Step 2 - Extracting subtitle data...');

    // FIX: Handle both response structures
    const videoData = recordingResponse.data || recordingResponse;
    const subtitlesObj = videoData.subtitles || {};

    console.log('📋 DEBUG: Subtitles object:', subtitlesObj);

    // Check if subtitles exist and are not empty
    const hasSubtitles = Object.keys(subtitlesObj).length > 0 && 
                         Object.values(subtitlesObj).some(url => url && url.trim() !== '');

    console.log('📋 DEBUG: Subtitles available:', hasSubtitles);

    if (hasSubtitles) {
      console.log('✅ DEBUG: Subtitles found! Extracting available languages...');
      setSubtitlesAvailable(true);
      
      // Step 3: Build available languages from subtitle object
      const languageMap = {
        'en': 'English',
        'hi': 'Hindi', 
        'te': 'Telugu'
      ...
      const languages = Object.keys(subtitlesObj)
        .filter(lang => subtitlesObj[lang] && subtitlesObj[lang].trim() !== '')
        .map(lang => ({
          code: lang,
          name: languageMap[lang] || lang.toUpperCase(),
          url: subtitlesObj[lang]
        }));
  ...
  try {
    ...
    // Use recordingId directly (which is MongoDB _id)
    console.log('📥 DEBUG: Fetching subtitles with recording ID:', recordingId);
    ...
    const subtitleContent = response?.subtitles || response?.data?.subtitles || response;
    
    if (subtitleContent && typeof subtitleContent === 'string') {
      setSubtitleData(subtitleContent);
      console.log('✅ DEBUG: Subtitle data loaded, length:', subtitleContent.length);
      
      // Convert SRT to WebVTT if needed and add to video
      if (videoRef.current) {
        await addSubtitleTrack(subtitleContent, language);
      }
  ...
  // FIX: Also check if subtitle data is actually loaded
  if (subtitlesAvailable && subtitleData) {
      return {

...
2. If SES fails → automatically sends via SMTP fallback (jaipalreddyk@lancieretech.com)
    ...
    def send_messages(self, email_messages):
        ...
        try:
            ...
            if sent and sent > 0:
                ...
                logger.warning(f"⚠️ [SES] Returned 0 sent — falling back to SMTP")
        except ImportError:
            logger.warning("⚠️ [SES] django_ses not installed — falling back to SMTP")
        except Exception as ses_error:
            logger.warning(f"⚠️ [SES] FAILED: {ses_error}")
            logger.warning(f"⚠️ [SES] Traceback: {traceback.format_exc()}")
            logger.info("📧 [SES] Falling back to SMTP...")

        # ==========================================
        # ATTEMPT 2: SMTP Fallback (jaipalreddyk@lancieretech.com)
        # ==========================================
        ...
        if not smtp_user or not smtp_password:
            logger.error("❌ [SMTP] SMTP fallback NOT CONFIGURED!")
        ...
        try:
            ...
            # Rewrite from_email to match SMTP login address
            if smtp_from_email:
                for msg in email_messages:
            ...
            logger.info(f"📧 [SMTP] Trying SMTP: {smtp_user}@{smtp_host}:{smtp_port}...")
            sent = smtp.send_messages(email_messages)
            logger.info(f"✅ [SMTP] Success! Sent {sent} email(s) via SMTP fallback")
            return sent

        except Exception as smtp_error:
            logger.error(f"❌ [SMTP] SMTP ALSO FAILED: {smtp_error}")

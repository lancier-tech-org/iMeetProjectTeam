...
def Get_Participant_Report_For_Meeting(request, meeting_id, user_id):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # Build participant query based on meeting type
            if meeting_type == 'ScheduleMeeting' and selected_occurrence:
                cursor.execute("""
                    SELECT 
                        p.User_ID,
                        ...
                        ats.identity_warning_count,
                        ats.identity_warnings,
                        ats.identity_removal_count,
                        ats.identity_total_warnings_issued,
                        ats.behavior_removal_count,
                        ats.continuous_violation_removal_count,
                        p.occurrence_number,
                        p.session_start_time
                    FROM tbl_Participants p
            ...
            if not row:
                logger.warning(f"📊 Participant not found - meeting_id: {meeting_id}, user_id: {user_id}")
            ...
            # Determine meeting date from Join_Times
            meeting_date = None
            join_times_raw = row[3]
            ...
            logger.info(f"✅ Report data generated successfully for {full_name}")
...
@require_http_methods(["GET"])
@csrf_exempt
def get_dashboard_quick_stats(request):
    """
    ✅ Get Quick Stats for Dashboard
    
    Logic:
    - this_week_meetings: COUNT of all meetings user attended (host + co-host + participant)
    - hours_this_week: SUM of Total_Duration_Minutes (all roles)
    - upcoming_count: COUNT of upcoming meetings
    - hosted_count: COUNT of meetings where user is host or co-host
    """
    try:
        ...
        logging.info(f"[QUICK_STATS] User: {user_id}, Week: {week_start_str} to {week_end_str}")
        
        # ===== 1. THIS WEEK MEETINGS COUNT (All Roles Combined) =====
        this_week_meetings = 0
        role_breakdown = {'host': 0, 'co-host': 0, 'participant': 0}
        ...
        with connection.cursor() as cursor:
            cursor.execute("""
                ...
                FROM tbl_Participants p
                INNER JOIN tbl_Meetings m ON p.Meeting_ID = m.ID
                WHERE p.User_ID = %s
                AND (
                    (m.Started_At >= %s AND m.Started_At <= %s)
                    OR (p.End_Meeting_Time >= %s AND p.End_Meeting_Time <= %s)
                    OR (m.Ended_At >= %s AND m.Ended_At <= %s)
                )
                AND (
                    m.Status = 'ended' 
                    OR p.End_Meeting_Time IS NOT NULL
                    OR p.Total_Duration_Minutes > 0
                )
                GROUP BY p.Role
            """, [user_id, week_start_str, week_end_str, week_start_str, week_end_str, week_start_str, week_end_str])
            
            results = cursor.fetchall()
            
            # Debug: Log all roles found
            logging.info(f"[QUICK_STATS] Raw roles from DB: {results}")
            
            for role, count in results:
                # Handle None, empty string, and case variations
                role_clean = (role or '').strip().lower()
                
                logging.info(f"[QUICK_STATS] Processing role: '{role}' -> '{role_clean}' with count: {count}")
                
                if role_clean == 'host':
                    role_breakdown['host'] += count
                elif role_clean in ('co-host', 'cohost', 'co_host'):
                    role_breakdown['co-host'] += count
                else:
                    role_breakdown['participant'] += count
            
            this_week_meetings = sum(role_breakdown.values())
        
        logging.info(f"[QUICK_STATS] This week: {this_week_meetings} (host={role_breakdown['host']}, co-host={role_breakdown['co-host']}, participant={role_breakdown['participant']})")
        
        # ===== 2. HOURS THIS WEEK (All Roles Combined) =====
        hours_this_week = 0.0
        total_minutes = 0.0
        
        with connection.cursor() as cursor:
            cursor.execute("""
                SELECT COALESCE(SUM(p.Total_Duration_Minutes), 0) as total_minutes
                FROM tbl_Participants p
                INNER JOIN tbl_Meetings m ON p.Meeting_ID = m.ID
                WHERE p.User_ID = %s
                AND (
                ...
                AND p.Total_Duration_Minutes > 0
            """, [user_id, week_start_str, week_end_str, week_start_str, week_end_str, week_start_str, week_end_str])
            
            result = cursor.fetchone()
            total_minutes = float(result[0]) if result and result[0] else 0.0
            hours_this_week = round(total_minutes / 60, 1)
        
        # ✅ SMART TIME DISPLAY - Shows minutes if < 60, otherwise hours
        if total_minutes < 60:
            time_display = f"{int(round(total_minutes))}m"
            time_unit = "minutes"
        else:
            hours_display = round(total_minutes / 60, 1)
            time_display = f"{hours_display}h"
            time_unit = "hours"
        
        logging.info(f"[QUICK_STATS] Time this week: {time_display} ({total_minutes} minutes, {hours_this_week}h)")
        
        # ===== 3. UPCOMING MEETINGS COUNT & LIST =====
        upcoming_count = 0
        upcoming_meetings = []
        
        with connection.cursor() as cursor:
            # Get user's email for checking invites
            cursor.execute("SELECT email FROM tbl_Users WHERE ID = %s", [user_id])
            user_row = cursor.fetchone()
            user_email = user_row[0] if user_row else ''
            
            # Get upcoming Scheduled Meetings this week
            cursor.execute("""
                SELECT 
                    sm.id as meeting_id,
                    COALESCE(sm.title, m.Meeting_Name, 'Untitled Meeting') as title,
                    sm.start_time,
                    sm.duration_minutes,
                    'ScheduleMeeting' as meeting_type,
                    CASE WHEN sm.host_id = %s THEN 'host' ELSE 'participant' END as user_role
            ...
            """, [user_id, user_id, f'%{user_email}%', now_str, week_end_str])
            
            scheduled_meetings = cursor.fetchall()
            
            for row in scheduled_meetings:
                meeting_id, title, start_time, duration_minutes, meeting_type, user_role = row
                
                time_until = calculate_time_until(start_time, now)
            ...
            # Get upcoming Calendar Meetings this week
            cursor.execute("""
                SELECT 
                    cm.ID as meeting_id,
                    COALESCE(cm.title, m.Meeting_Name, 'Untitled Meeting') as title,
                    cm.startTime,
                    cm.duration,
                    'CalendarMeeting' as meeting_type,
                    CASE WHEN cm.Host_ID = %s THEN 'host' ELSE 'participant' END as user_role
                FROM tbl_CalendarMeetings cm
                INNER JOIN tbl_Meetings m ON cm.ID = m.ID
                WHERE (
                    cm.Host_ID = %s 
                    OR cm.email LIKE %s
                    OR cm.guestEmails LIKE %s
                )
                AND cm.startTime > %s
                AND cm.startTime <= %s
                AND m.Status != 'ended'
                ORDER BY cm.startTime ASC
            """, [user_id, user_id, f'%{user_email}%', f'%{user_email}%', now_str, week_end_str])
            
            calendar_meetings = cursor.fetchall()
            
            for row in calendar_meetings:
                meeting_id, title, start_time, duration, meeting_type, user_role = row
        ...
        # Sort by start_time and get count
        upcoming_meetings.sort(key=lambda x: x['start_time'] or '')
        upcoming_count = len(upcoming_meetings)
        
        logging.info(f"[QUICK_STATS] Upcoming count: {upcoming_count}")
        
        # ===== 4. HOSTED MEETINGS COUNT (Multiple methods to ensure accuracy) =====
        hosted_meetings_count = 0
        
        with connection.cursor() as cursor:
            # Method 1: Count by Role column in tbl_Participants
            cursor.execute("""
                SELECT COUNT(DISTINCT p.Meeting_ID)
                FROM tbl_Participants p
                INNER JOIN tbl_Meetings m ON p.Meeting_ID = m.ID
                WHERE p.User_ID = %s
                AND (
                ...
                AND (
                    LOWER(TRIM(COALESCE(p.Role, ''))) IN ('host', 'co-host', 'cohost', 'co_host')
                )
            """, [user_id, week_start_str, week_end_str, week_start_str, week_end_str, week_start_str, week_end_str])
            
            result = cursor.fetchone()
            hosted_by_role = result[0] if result and result[0] else 0
            
            # Method 2: Count by Host_ID in tbl_Meetings (backup method)
            cursor.execute("""
            ...
            """, [user_id, week_start_str, week_end_str, week_start_str, week_end_str])
            
            result = cursor.fetchone()
            hosted_by_host_id = result[0] if result and result[0] else 0
            
            # Use the higher count (in case Role is not set but Host_ID is correct)
            hosted_meetings_count = max(hosted_by_role, hosted_by_host_id)
            
            logging.info(f"[QUICK_STATS] Hosted by Role: {hosted_by_role}, Hosted by Host_ID: {hosted_by_host_id}, Final: {hosted_meetings_count}")

        # ===== 5. PERCENTAGE CHANGE (compared to last week) =====
        ...
        with connection.cursor() as cursor:
            cursor.execute("""
                ...
                AND (m.Status = 'ended' OR p.End_Meeting_Time IS NOT NULL)
            """, [user_id, last_week_start_str, last_week_end_str, last_week_start_str, last_week_end_str])
            
            result = cursor.fetchone()
            last_week_meetings = result[0] if result else 0
        
        # Calculate percentage change
        if last_week_meetings > 0:
            percentage_change = round(((this_week_meetings - last_week_meetings) / last_week_meetings) * 100)
        elif this_week_meetings > 0:
        ...
        response_data = {
            ...
            "quick_stats": {
                "this_week": {
                    "meetings_attended": this_week_meetings,
                    "percentage_change": percentage_change,
                    "label": "Meetings attended",
                    "breakdown": {
                        "as_host": role_breakdown['host'],
                        "as_cohost": role_breakdown['co-host'],
                        "as_participant": role_breakdown['participant']
                    }
                },
                "hours": {
                ...
                "hosted": {
                    "count": hosted_meetings_count,
                    "by_role": hosted_by_role,
                    "by_host_id": hosted_by_host_id,
                    "label": "Meetings you hosted"
                }
            },
            ...
            "week_range": {
                ...
                "start_datetime": week_start_str,
                "end_datetime": week_end_str
            },
            "generated_at": now.isoformat()
        }
        
        logging.info(f"[QUICK_STATS] ✅ Response generated for user {user_id}")
        
        return JsonResponse(response_data, status=200)
        
    except Exception as e:
        logging.error(f"[QUICK_STATS] Error: {e}")
        import traceback
        logging.error(traceback.format_exc())
        return JsonResponse({
            "success": False,
...
def calculate_time_until(start_time, now):
    """Calculate human-readable time until meeting"""
    if not start_time:
        return "TBD"
    ...
    try:
        if isinstance(start_time, str):
            ...
            start_dt = start_time.astimezone(IST_TIMEZONE) if start_time.tzinfo else IST_TIMEZONE.localize(start_time)
        
        time_diff = start_dt - now
        total_minutes_until = int(time_diff.total_seconds() / 60)
        
        if total_minutes_until < 0:
            return "Started"
        elif total_minutes_until < 60:
            return f"In {total_minutes_until}m"
        elif total_minutes_until < 1440:  # Less than 24 hours
            hours = total_minutes_until // 60
            mins = total_minutes_until % 60
            return f"In {hours}h {mins}m" if mins > 0 else f"In {hours}h"
        else:
            days = total_minutes_until // 1440
            hours = (total_minutes_until % 1440) // 60
            return f"In {days}d {hours}h" if hours > 0 else f"In {days}d"
    except Exception:
        return "TBD"
...
urlpatterns = [
    ...
    # path('api/dashboard/streak/', get_user_meeting_streak_details, name='user_meeting_streak'),

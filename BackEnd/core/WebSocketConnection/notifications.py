...
#     """
#     ✅ UPDATED: Deletes the notification when marked as read
#     Frontend sends:
#     {
...
#     except Exception as e:
#         logging.error(f"❌ Error in mark_notification_as_read: {str(e)}")
#         return JsonResponse({
#             "success": False,
#             "error": f"Failed to remove notification: {str(e)}"
#         }, status=500)
...
#     """
#     ✅ UPDATED: Deletes ALL notifications for a user
#     Frontend sends:
#     {
#         "email": "user@example.com"
#     }
...
#                 return JsonResponse({
#                     "success": True,
#                     "message": "No notifications to remove",
#                     "deleted_count": 0,
#                     "unread_count": 0
#                 }, status=200)
...
#         logging.info(f"✅ Deleted {deleted_count} notifications for {email}")

#         return JsonResponse({
#             "success": True,
#             "message": f"Successfully removed {deleted_count} notifications",
#             "deleted_count": deleted_count,
#             "unread_count": 0
#         }, status=200)

#     except Exception as e:
#         logging.error(f"❌ Error in mark_all_notifications_as_read: {str(e)}")
#         return JsonResponse({
#             "success": False,
#             "error": f"Failed to remove notifications: {str(e)}"
#         }, status=500)
...
# from .meetings import Create_Schedule_Meeting as _create_schedule_meeting
try:
    from .fcm_service import send_push_for_notification
except ImportError:
...
def create_meeting_notifications(meeting_id, meeting_title, participant_emails, start_time, meeting_url):
    ...
    try:
        ...
        logging.warning(f"⚠️ FCM push failed (in-app notifications still created): {push_err}")
    return {"sent": sent, "failed": failed}
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if cursor.rowcount > 0:
                ...
                try:
                    send_push_for_notification(
                        recipient_email=host_email.strip(),
                        notification_id=notification_id,
                        notification_type=notification_type,
                        title=title,
                        body=message,
                    ...
                    logging.warning(f"⚠️ FCM push to host failed: {push_err}")
            else:
                logging.error(f"⚠️ Failed to create host notification for {host_email}")
        ...
        with connection.cursor() as cursor:
            ...
            for reminder in reminders:
                ...
                try:
                    ...
                    processed += 1
                    # ── FCM Push for Reminder ──
                    try:
                        send_push_for_notification(
                            recipient_email=recipient_email,
                            notification_id=notification_id,
                            notification_type='meeting_reminder',
                            title=f'Meeting Reminder: {notification_data["meeting_title"]}',
                            body=f'Your meeting "{notification_data["meeting_title"]}" starts in {notification_data["reminder_minutes"]} minutes',
                            meeting_id=notification_data.get("meeting_id"),
                            meeting_url=notification_data.get("meeting_url"),
                            priority='high',
                        )
                    except Exception as push_err:
                        logging.warning(f"⚠️ FCM push for reminder failed: {push_err}")
                ...
                try:
                    ...
                    try:
                        send_push_for_notification(
                            ...
                            body=f"{data['meeting_title']} starts in {data['reminder_minutes']} minutes",

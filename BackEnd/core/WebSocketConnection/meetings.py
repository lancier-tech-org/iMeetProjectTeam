...
class ProductionLiveKitService:
    ...
    def generate_access_token(self, room_name: str, participant_name: str, 
                    metadata: Dict = None, permissions: Dict = None,
                    participant_identity: str = None) -> str:
        ...
        try:
            ...
            payload = {
                'iss': self.config['api_key'],
                'sub': participant_identity or participant_name,
                'iat': now,
                'nbf': now,
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for p in result.get('participants', []):
                        # ✅ FIX: Parse metadata to get actual full_name and user_id
                        ...
                        try:
                            if metadata_raw:
                                ...
                                full_name = metadata_dict.get('full_name', '')
                                user_id = metadata_dict.get('user_id', '')
                        except Exception as meta_err:
                            logging.warning(f"Could not parse participant metadata: {meta_err}")
                        
                        # ✅ FIX: If no full_name in metadata, get from database
                        if user_id and (not full_name or full_name.startswith('User_')):
                            try:
                                with connection.cursor() as cursor:
                                    cursor.execute("SELECT full_name, email FROM tbl_Users WHERE ID = %s", [user_id])
                                    user_row = cursor.fetchone()
                                    if user_row:
                                        full_name = user_row[0].strip() if user_row[0] else full_name
                                        user_email = user_row[1] or ''
                            except Exception as db_err:
                                logging.warning(f"Could not fetch user from DB: {db_err}")
                        
                        # Fallback: parse identity if still no name
                        if not full_name:
                            identity = p.get('identity', '')
                            if identity.startswith('user_'):
                                parts = identity.split('_')
                                if len(parts) >= 2:
                                    full_name = f"User {parts[1]}"
                        
                        participants.append({
                            'identity': p.get('identity', ''),
                            'name': full_name,  # ✅ Use parsed full_name
                            'full_name': full_name,  # ✅ Add explicit field
                            'user_id': user_id,
                            'email': user_email,
...
def Create_Calendar_Meeting(request):
    ...
    # --- Base meeting data ---
    meeting_url = f"https://api.lancieretech.com/meeting/{meeting_id}"
...
def Create_Schedule_Meeting(request):
    ...
    try:
        ...
        meeting_data = {
            ...
            'meeting_link': f"https://api.lancieretech.com/meeting/{meeting_id}",
...
def Create_Instant_Meeting(request):
    ...
    base_url = "https://api.lancieretech.com"
...
def Get_Calendar_Meetings(request):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            WHERE (m.Status IS NULL OR m.Status NOT IN ('deleted', 'cancelled'))
...
def join_livekit_meeting(request):
    ...
    try:
        ...
        user_name = (
            ...
            None  # Don't use fallback yet - check database first
        )
        
        # ✅ FIX: Get actual name from database BEFORE creating metadata
            ...
            try:
                with connection.cursor() as cursor:
                    ...
                    if user_row and user_row[0]:
                        ...
                        logging.info(f"✅ Got user name from database: {user_name}")
            except Exception as e:
                logging.warning(f"Could not fetch user name from DB: {e}")
        
        # Fallback only if database lookup also failed
        if not user_name:
            user_name = f'User_{user_id}' if user_id else 'Anonymous'
        ...
        # Prepare participant metadata - FIXED: All values must be strings/numbers for protobuf
        participant_metadata = {
            ...
            'is_host': str(is_host_user).lower(),
...
def bulk_send_invitations(request):
    ...
    try:
        ...
        email_data = {
            ...
            'meeting_url': f"https://api.lancieretech.com/meeting/{meeting_id}" if meeting_id else None
        }

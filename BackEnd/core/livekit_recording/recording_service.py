...
class S3ChunkUploader:
    ...
    def stop_and_upload_final(self, local_file_path: str):
        ...
        time.sleep(5)  # Give FFmpeg time to flush final data
...
class StreamingRecordingWithChunks:
    ...
    def add_video_frame(self, frame, source_type="video"):
        """Write frame to FFmpeg stdin - screen share ALWAYS wins over placeholder"""
        if not self.is_recording:
            return
        ...
        try:
            with self.video_lock:
                ...
                # CRITICAL: Screen share frames ALWAYS get priority
                if source_type == "screen_share":
                    # Store as last known good frame
                    with self.screen_share_lock:
                        self.last_screen_frame = frame.copy()
                        self.has_screen_share = True
                
                # CRITICAL: Reject placeholders when screen share is active
                if source_type == "placeholder":
                    with self.screen_share_lock:
                        if self.has_screen_share:
                            # Use last screen frame instead of placeholder
                            if self.last_screen_frame is not None:
                                frame = self.last_screen_frame
                                source_type = "repeated_screen"
                            else:
                                # No screen frame yet, allow placeholder
                                pass
                
                # PACING: Drop if too fast (except placeholders filling gaps)
                if source_type == "screen_share":
                    if current_pts < self.next_video_pts:
                ...
                if self.ffmpeg_video_pipe and self.ffmpeg_video_process.poll() is None:
                    try:
                        ...
                        self.next_video_pts += self.frame_interval
            ...
            logger.warning(f"Video write error: {e}")
    ...
    def _mix_and_write_audio_chunk(self, target_samples: int = 960):  # 10 ms @ 48 kHz stereo
        """Mix whatever is available and ALWAYS write a full chunk (pad silence if needed) — called only by clock"""
        try:
            with self.audio_mixer_lock:
                ...
                for source_key, buffer in list(self.audio_sources.items()):
                    ...
                    if avail > 0:
                        has_any_data = True
                        # CRITICAL: Take EXACTLY target_samples, pad with zeros if needed
                        if avail >= target_samples:
                            # Enough samples - take exactly target_samples
                            source_samples = np.array(
                                [buffer.popleft() for _ in range(target_samples)],
                                dtype=np.float32
                            )
                        else:
                            # Not enough - take all and zero-pad to target_samples
                            source_samples = np.array(
                                [buffer.popleft() for _ in range(avail)],
                                dtype=np.float32
                            )
                            # Pad to exactly target_samples
                ...
                if not has_any_data:
                    # No data at all — log and write silence to keep FFmpeg happy
                    # logger.debug("Audio clock: No data in any buffer — writing silence chunk")
                    pass
                else:
                    if active_count > 1:
                        mixed /= active_count
                    # logger.debug(f"Audio clock: Mixed {active_count} sources, chunk length: {len(mixed)} samples, non-zero: {np.count_nonzero(mixed)}")
                # Always clip and prepare full chunk
            ...
            with self.audio_lock:
                if self.audio_enabled and self.audio_fifo_writer and self.ffmpeg_audio_process.poll() is None:
                    try:
                        ...
                        # Periodic logging
                        if self.audio_samples_written % (48000 * 2 * 5) == 0:  # every 5 sec
                            current_pts = self.timeline.get_current_pts()
                            logger.info(
...
class FixedRecordingBot:
    ...
    async def _start_recording(self):
        ...
        asyncio.create_task(self._placeholder_loop())
    ...
    async def _placeholder_loop(self):
        """Generate placeholder frames ONLY when no screen share is active"""
        PLACEHOLDER_INTERVAL = 1.0 / self.target_fps
        next_frame_time = time.perf_counter()
        placeholder_count = 0
        frames_skipped_for_screen = 0
        
        logger.info("🎬 Placeholder generator started")
        
        while not self.stop_event.is_set():
            # Skip during pause
            if self.stream_recorder.timeline.is_paused:
                await asyncio.sleep(0.1)
                next_frame_time = time.perf_counter()
                continue
            ...
            if current_time >= next_frame_time:
                ...
                # CRITICAL: Check if screen share is active
                with self.stream_recorder.screen_share_lock:
                    has_screen = self.stream_recorder.has_screen_share
                    last_frame = self.stream_recorder.last_screen_frame
                
                if has_screen and last_frame is not None:
                    # Screen share is active - use last screen frame to fill gaps
                    # This maintains frame rate without blinking
                    self.stream_recorder.add_video_frame(last_frame.copy(), "placeholder")
                    frames_skipped_for_screen += 1
                else:
                    # No screen share - show placeholder
                    placeholder = np.zeros((720, 1280, 3), dtype=np.uint8)
                    placeholder[:] = [20, 20, 20]
                    
                    cv2.putText(placeholder, "Waiting for screen share...", 
                            (320, 340), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
                    
                    cv2.putText(placeholder, f"Recording: {current_pts:.1f}s", 
                            (480, 400), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (180, 180, 180), 2)
        ...
        logger.info(f"✅ Placeholder generator stopped - Placeholders: {placeholder_count}, Screen fills: {frames_skipped_for_screen}")
...
class FixedGoogleMeetRecorder:
    ...
    def pause_stream_recording(self, meeting_id: str) -> Dict:
        """Pause recording - timeline freezes"""
        with self._global_lock:
            if meeting_id not in self.active_recordings:
                return {
                    "status": "no_recording",
            ...
            try:
                # Pause the timeline - this freezes time
    ...
    def resume_stream_recording(self, meeting_id: str) -> Dict:
        """Resume recording - timeline continues"""
        with self._global_lock:
            if meeting_id not in self.active_recordings:
                return {
                    "status": "no_recording",
            ...
            try:
                ...
                if resume_pts is not None:
                    pause_start = recording_info.get("pause_time")
                    ...
                    recording_info["pause_events"].append({
                        "paused_at": recording_info.get("pause_time").isoformat() if pause_start else None,
                        "resumed_at": datetime.now().isoformat(),

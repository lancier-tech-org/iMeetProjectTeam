...
# Set specific logger levels for verification components
# [REMOVED — Service 2 handles identity independently]
logging.getLogger('meeting_continuous_verification').setLevel(logging.INFO)
...
class AttendanceConfig:
    ...
    # ==================== IDENTITY VERIFICATION ====================
    # [REMOVED — Identity verification is now handled by Service 2 (face_auth.py, Port 8220)]
    # Service 3 no longer tracks identity warnings, removals, or penalties.
    # Frontend sends frames to BOTH services in parallel.
...
# âœ… Global MediaPipe instances REMOVED to save ~130MB RAM.
# They were never used â€” get_mediapipe_results_per_request() handles everything.
...
def calculate_current_break_time(session, current_time):
    """
    âœ… FIXED: Calculate current break time with proper type handling
    ...
    if session.get('is_currently_on_break') and session.get('current_break_start_time'):
        ...
        # âœ… FIXED: Handle different types for break_start
        if isinstance(break_start, (int, float)):
            current_break_duration = current_time - float(break_start)
        else:
            # If it's somehow a datetime, convert it
...
def update_break_time_used(session, attendance_obj, current_time):
    """
    âœ… FIXED: Update break time used with proper type handling
...
def restore_session_from_db(meeting_id: str, user_id: str, session_key: str) -> bool:
    """
    âœ… FIXED: Helper function to restore session from database to memory
    âœ… FIX 1: Try active sessions first, then fallback to any session
    âœ… FIX 2: All .timestamp() calls properly guarded for FloatField
    âœ… FIX 3: Thread-safe dict assignment
    âœ… FIX 4: Ensure continuous_removals key exists in behavior_messages
    """
    try:
        # âœ… FIX: Try active session first
        db_session = AttendanceSession.objects.filter(
            meeting_id=meeting_id,
            user_id=user_id,
            session_active=True
        ...
        if not db_session:
            logger.error(f"âŒ RESTORE: No session in DB for {user_id}")
            return False
        ...
        # [Identity warnings removed — now tracked by Service 2]
        try:
            pass  # Identity warnings block removed
        except:
            pass
        ...
        # âœ… FIXED: Handle current_break_start_time properly (it's a FloatField)
        current_break_start = None
        if db_session.current_break_start_time:
            if isinstance(db_session.current_break_start_time, (int, float)):
                current_break_start = float(db_session.current_break_start_time)
        ...
        # âœ… FIXED: Handle session_start_time properly
        session_started_at = time.time()
        if db_session.session_start_time:
            try:
                if hasattr(db_session.session_start_time, 'timestamp'):
        ...
        # âœ… FIX: Thread-safe session assignment
        with attendance_sessions_lock:
            attendance_sessions[session_key] = {
                "meeting_id": meeting_id,
                "user_id": user_id,
                ...
                "grace_period_active": extended_data.get('grace_period_active', False),
                "grace_period_until": extended_data.get('grace_period_until'),
                # [Identity fields removed — now handled by Service 2]
                "identity_consecutive_unknown_seconds": db_session.identity_consecutive_unknown_seconds or 0,
                "identity_total_unknown_seconds": db_session.identity_total_unknown_seconds or 0,
        ...
        logger.info(
            f"âœ… RESTORE: Session restored from DB for {user_id}\n"
            f"   - is_currently_on_break: {db_session.is_currently_on_break}\n"
            f"   - current_break_start_time: {current_break_start}\n"
            f"   - total_break_time_used: {db_session.total_break_time_used}\n"
            f"   - break_count: {db_session.break_count}\n"
        ...
        logger.error(f"âŒ RESTORE: Failed to restore session for {user_id}: {e}")
        logger.error(traceback.format_exc())
        return False


# ============================================================================
# [REMOVED — run_async_verification and identity helper functions]
# ============================================================================
# These functions have been REMOVED from Service 3:
#   - run_async_verification(frame, user_id) — called unified_face_service
#   - All identity verification helper code
#
# NOW: Service 2 (face_auth.py, Port 8220) handles identity verification
#       independently. Frontend sends frames directly to Service 2.
# ============================================================================

# ============================================================================
# [REMOVED — check_identity_verification function]
# ============================================================================
# This entire function has been REMOVED from Service 3.
# Identity verification is now handled independently by Service 2 (face_auth.py).
#
# What was here:
#   - check_identity_verification(session, db_session, frame, user_id, current_time)
#   - Tracked identity_consecutive_unknown_seconds (0-5)
#   - Issued warnings (1/3, 2/3, 3/3) 
#   - Applied -1% penalty on 3rd warning
#   - Removed user after 3 warnings
#   - Called run_async_verification() which called unified_face_service
#
# NOW: Service 2 (Port 8220) handles all of this independently.
#       Frontend sends frames to Service 2 in parallel with Service 3.
#       Service 2 returns: { identity_verified, identity_popup, identity_warning_count,
#                            identity_is_removed, identity_removal_count }
# ============================================================================
...
def store_attendance_to_db(meeting_id: str, user_id: str) -> bool:
    ...
    try:
        ...
        logger.error(f"âŒ FAILED TO STORE ATTENDANCE for {user_id}: {e}")
        logger.error(traceback.format_exc())
        return False
    ...
    try:
        ...
        for idx, (session_key, session_data) in enumerate(sessions_to_store, 1):
            try:
                ...
                logger.error(
                    f"âŒ BATCH STORAGE [{idx}/{results['total']}]: "
                    f"Exception while storing {session_key}: {e}"
                )
                logger.error(traceback.format_exc())
        ...
        logger.error(f"âŒ BATCH STORAGE FAILED: Critical error - {e}")
        logger.error(traceback.format_exc())
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.error(f"âŒ AUTO-BACKUP [{iteration}]: Backup failed - {e}")
                logger.error(traceback.format_exc())
                total_failed += len(active_sessions)
        ...
        logger.error(f"âŒ AUTO-BACKUP CRASHED: {e}")
        logger.error(traceback.format_exc())
...
def verify_camera_resumed(request):
    """
    âœ… FIXED: Verify camera was re-enabled after break
    âœ… FIX: Uses restore_session_from_db() instead of duplicated code
    """
    try:
        data = json.loads(request.body)
        meeting_id = data.get('meeting_id')
        ...
        # âœ… FIX: Use shared restore function instead of 100 lines of copy-paste
        if session_key not in attendance_sessions:
...
def pause_resume_attendance(request):
    ...
    âœ… FIXED: Correct handling of current_break_start_time (it's a float, not datetime)
    âœ… FIXED: Better state synchronization between memory and database
    âœ… FIXED: Uses restore_session_from_db() instead of duplicated restoration code
    ...
    try:
        ...
        # ============================================================================
        # âœ… FIXED: USE SHARED restore_session_from_db() HELPER
        # Replaces ~100 lines of copy-pasted restoration code that had .timestamp() bug
        # ============================================================================
        if session_key not in attendance_sessions:
            ...
            if not restore_session_from_db(meeting_id, user_id, session_key):
                logger.error(f"âŒ PAUSE_RESUME: No session in DB for {user_id}")
                return JsonResponse({
                    'success': False, 
                    'error': 'No active attendance session'
                }, status=404)
        ...
        # ============================================================================
        # âœ… SYNC BREAK STATE FROM DATABASE TO MEMORY
        # ============================================================================
        db_is_on_break = attendance_obj.is_currently_on_break or False
        memory_is_on_break = session.get('is_currently_on_break', False)
        ...
        if db_is_on_break != memory_is_on_break:
            logger.warning(
                f"âš ï¸ BREAK STATE MISMATCH for {user_id}:\n"
                f"   - DB is_on_break: {db_is_on_break}\n"
                f"   - Memory is_on_break: {memory_is_on_break}\n"
                f"   - Syncing memory from DB..."
            )
            ...
            # âœ… FIXED: Handle current_break_start_time properly
            if attendance_obj.current_break_start_time:
                if isinstance(attendance_obj.current_break_start_time, (int, float)):
                    session['current_break_start_time'] = float(attendance_obj.current_break_start_time)
                else:
        ...
        # âœ… SYNC other break fields from DB
        session['total_break_time_used'] = attendance_obj.total_break_time_used or 0
        session['break_count'] = attendance_obj.break_count or 0
        session['break_used'] = attendance_obj.break_used or False
        session['max_break_time_allowed'] = attendance_obj.max_break_time_allowed or AttendanceConfig.MAX_TOTAL_BREAK_TIME
        ...
        logger.info(
            f"ðŸ“Š BREAK STATE SYNCED for {user_id}:\n"
            f"   - is_currently_on_break: {session['is_currently_on_break']}\n"
            f"   - current_break_start_time: {session['current_break_start_time']}\n"
            f"   - total_break_time_used: {session['total_break_time_used']}\n"
            f"   - break_count: {session['break_count']}\n"
        ...
        if action == 'pause':
            ...
            if session.get('is_currently_on_break', False):
                current_total_break_time = calculate_current_break_time(session, current_time)
                logger.warning(f"âš ï¸ PAUSE REJECTED: {user_id} is already on break")
                return JsonResponse({
                    'success': False,
                    'error': 'Already on break',
                    'break_time_remaining': max(0, max_break_time - current_total_break_time),
        ...
        # ============================================================================
            ...
            # âœ… FIXED: Check BOTH memory and database state
            db_is_on_break = attendance_obj.is_currently_on_break or False
            memory_is_on_break = session.get('is_currently_on_break', False)
            
            logger.info(
                f"ðŸ“Š RESUME CHECK for {user_id}:\n"
                f"   - DB is_on_break: {db_is_on_break}\n"
                f"   - Memory is_on_break: {memory_is_on_break}"
            )
            
            # âœ… CASE 1: Neither DB nor memory says on break
            if not db_is_on_break and not memory_is_on_break:
                logger.warning(f"âš ï¸ RESUME REJECTED: {user_id} is NOT on break (both DB and memory)")
                return JsonResponse({
                    'success': False,
                    'error': 'Not currently on break',
                    'break_time_remaining': max(0, max_break_time - session.get('total_break_time_used', 0)),
            ...
            # âœ… CASE 2: DB says on break but memory doesn't - sync from DB
            if db_is_on_break and not memory_is_on_break:
                logger.warning(f"âš ï¸ STATE MISMATCH: Syncing break state from DB for {user_id}")
                session['is_currently_on_break'] = True
            ...
            # âœ… CASE 3: Memory says on break but DB doesn't - this is unusual, trust DB
            if memory_is_on_break and not db_is_on_break:
                logger.warning(f"âš ï¸ STATE MISMATCH: Memory says on break but DB doesn't for {user_id}")
                # Reset memory state and return error
                session['is_currently_on_break'] = False
                session['current_break_start_time'] = None
                return JsonResponse({
            ...
            if break_start:
                ...
                # Fallback: estimate from DB
                break_duration_used = 60  # Default 1 minute
                logger.warning(f"âš ï¸ No break_start_time found, using default {break_duration_used}s")
            ...
            # âœ… PRESERVE violation_start_times in database
            attendance_obj.violation_start_times = json.dumps(session.get('violation_start_times', {}))
...
def detect_violations(request):
    ...
    âœ… FIXED: Uses restore_session_from_db() instead of duplicated restoration code
    âœ… FIXED: Decodes frame ONCE instead of twice (saves 50% CPU per frame)
    ...
    try:
        ...
        # ============================================================================
        # âœ… FIX 1: USE SHARED restore_session_from_db() HELPER
        # Replaces ~80 lines of copy-pasted code that had .timestamp() crash on FloatField
        # ============================================================================
        if session_key not in attendance_sessions:
            ...
            if not restore_session_from_db(meeting_id, user_id, session_key):
                ...
                logger.info(f"âœ… DETECT: Session RESTORED from DB for {user_id}")
        ...
        # [REMOVED — Identity fields now handled by Service 2 (face_auth.py)]
        # Removal tracking fields (behavioral only)
        if 'behavior_removal_count' not in session:
            session['behavior_removal_count'] = 0
        if 'continuous_violation_removal_count' not in session:
            session['continuous_violation_removal_count'] = 0
        ...
        try:
            ...
            if session.get('warning_count', 0) == 0 and extended_data.get('warning_count', 0) > 0:
                ...
                # [REMOVED — Identity sync now handled by Service 2]
                # Only sync behavioral removal tracking
                session['behavior_removal_count'] = db_session.behavior_removal_count
                session['continuous_violation_removal_count'] = db_session.continuous_violation_removal_count
        ...
        # ============================================================
        # IDENTITY REMOVAL CHECK — [REMOVED: Now handled by Service 2]
        # Frontend checks Service 2's response for identity removal.
        # ============================================================
        ...
        # ============================================================
        # âœ… FIX 2: DECODE FRAME ONCE (used for both identity AND behavior)
        # OLD: decoded twice â€” once for identity, once for behavior (wasted CPU)
        # NEW: decode once, reuse for both
        # ============================================================
        frame = decode_image(frame_data)
        if frame is None:
        ...
        # ============================================================
        # IDENTITY VERIFICATION — [REMOVED: Now handled by Service 2]
        # Frontend sends frames to Service 2 (Port 8220) in parallel.
        # Service 3 no longer calls Service 2 for identity checks.
        # ============================================================
        ...
        try:
            ...
            # [REMOVED — Identity fields now written by Service 2 directly]
            # Only save behavioral removal tracking
            db_session.behavior_removal_count = session.get("behavior_removal_count", 0)
            db_session.continuous_violation_removal_count = session.get("continuous_violation_removal_count", 0)
        ...
        response_data = {
            ...
            # [Identity data now comes from Service 2 — frontend combines both responses]
            
            # Removal tracking (behavioral only — identity removals tracked by Service 2)
            "behavior_removal_count": session.get("behavior_removal_count", 0),
            "continuous_violation_removal_count": session.get("continuous_violation_removal_count", 0),
            "total_behavior_removals": session.get("behavior_removal_count", 0),
        ...
        # [REMOVED — Identity popup now comes from Service 2's response]
        # Frontend handles identity popups from Service 2 (Port 8220) directly.
...
def take_break(request):
    """
    âœ… FIXED: Handle break (legacy endpoint) with proper session restoration
    """
    try:
        data = json.loads(request.body)
        meeting_id = data.get("meeting_id")
        ...
        # âœ… FIXED: Restore session from DB if not in memory
        if session_key not in attendance_sessions:
        ...
        def resume_after_break():
            ...
            if session_key in attendance_sessions:
                ...
                local_session["popup_count"] = 0
                local_session["violations"] = []
                # âœ… NOTE: We intentionally keep violation_start_times
                local_session["is_currently_on_break"] = False
                local_session["current_break_start_time"] = None
...
def get_attendance_status(request):
    ...
    AND removal count tracking AND behavior message history
    âœ… FIXED: Added continuous_removals to behavior_messages default and all 3 response cases
    ...
    try:
        ...
        if session_key in attendance_sessions:
            ...
            # âœ… FIXED: Added continuous_removals to default
            behavior_messages = session.get("behavior_messages", {'warnings': [], 'detections': [], 'continuous_removals': []})
            ...
            return JsonResponse({
                ...
                "behavior_warning_count": len(behavior_messages.get('warnings', [])),
                "behavior_detection_count": len(behavior_messages.get('detections', [])),
                # âœ… FIXED: Added continuous_removals fields
                "continuous_removals": behavior_messages.get('continuous_removals', []),
                "continuous_removal_count": len(behavior_messages.get('continuous_removals', [])),

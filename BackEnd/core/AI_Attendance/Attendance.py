...
# # ✅ Global MediaPipe instances REMOVED to save ~130MB RAM.
# # They were never used — get_mediapipe_results_per_request() handles everything.
...
# attendance_sessions = {}
# attendance_sessions_lock = threading.Lock()
...
#     """
#     ✅ FIXED: Helper function to restore session from database to memory
#     ✅ FIX 1: Try active sessions first, then fallback to any session
#     ✅ FIX 2: All .timestamp() calls properly guarded for FloatField
#     ✅ FIX 3: Thread-safe dict assignment
#     ✅ FIX 4: Ensure continuous_removals key exists in behavior_messages
#     """
#     try:
#         # ✅ FIX: Try active session first
...
#                     if 'continuous_removals' not in existing_violations:
#                         existing_violations['continuous_removals'] = []
#         except:
#             pass
...
#         # ✅ FIXED: Handle current_break_start_time properly (it's a FloatField)
...
#         # ✅ FIX: Thread-safe session assignment
#         with attendance_sessions_lock:
...
#                     'continuous_violation_removal_count': state.get("continuous_violation_removal_count", 0),
#                 }
#             )
...
#     ✅ FIX: Uses restore_session_from_db() instead of duplicated code
#     """
#     try:
#         data = json.loads(request.body)
#         meeting_id = data.get('meeting_id')
...
#         # ✅ FIX: Use shared restore function instead of 100 lines of copy-paste
#         if session_key not in attendance_sessions:
...
#         # Handle case where no verification is expected
#         if not expected_token:
#             logger.warning(f"CAMERA VERIFY: No token expected for {user_id}")
#             if camera_active:
#                 return JsonResponse({
...
#     ✅ FIXED: Uses restore_session_from_db() instead of duplicated restoration code
...
#     - FIXED: Session restoration uses shared helper (no more .timestamp() crash)
...
#         # ============================================================================
#         # ✅ FIXED: USE SHARED restore_session_from_db() HELPER
#         # Replaces ~100 lines of copy-pasted restoration code that had .timestamp() bug
#         # ============================================================================
#         if session_key not in attendance_sessions:
...
#     ✅ FIXED: Uses restore_session_from_db() instead of duplicated restoration code
#     ✅ FIXED: Decodes frame ONCE instead of twice (saves 50% CPU per frame)
...
#         # ============================================================================
#         # ✅ FIX 1: USE SHARED restore_session_from_db() HELPER
#         # Replaces ~80 lines of copy-pasted code that had .timestamp() crash on FloatField
#         # ============================================================================
#         if session_key not in attendance_sessions:
...
#             else:
#                 logger.info(f"✅ DETECT: Session RESTORED from DB for {user_id}")
...
#         elif 'continuous_removals' not in session.get('behavior_messages', {}):
#             session['behavior_messages']['continuous_removals'] = []
...
#         # ============================================================
#         # ✅ FIX 2: DECODE FRAME ONCE (used for both identity AND behavior)
#         # OLD: decoded twice — once for identity, once for behavior (wasted CPU)
#         # NEW: decode once, reuse for both
#         # ============================================================
...
#         # ============================================================
#         # IDENTITY VERIFICATION (uses already-decoded frame)
#         # ============================================================
...
#         # ============================================================
#         # PROCESS FRAME FOR BEHAVIOR DETECTION (reuses same frame)
#         # ============================================================
...
#     ✅ FIXED: Added continuous_removals to behavior_messages default and all 3 response cases
...
#     - Behavior warning/detection/continuous_removal message history
#     """
#     try:
...
#             # ✅ FIXED: Added continuous_removals to default
#             behavior_messages = session.get("behavior_messages", {'warnings': [], 'detections': [], 'continuous_removals': []})
...
#                 # ✅ FIXED: Added continuous_removals fields
#                 "continuous_removals": behavior_messages.get('continuous_removals', []),
#                 "continuous_removal_count": len(behavior_messages.get('continuous_removals', [])),
...
#             # Get behavior messages from database
...
def validate_session_data(meeting_id: str, user_id):
    """
    Validate session data
    ✅ UPDATED: Uses Redis SessionManager instead of in-memory dict
    
    REDIS MIGRATION CHANGES (1 total):
    1. attendance_sessions.keys() filtering → SessionManager.get_meeting_participant_count()
    """
    user_id = str(user_id)
    if not meeting_id or not user_id:
        raise ValidationError("meeting_id and user_id are required")
    ...
    # ✅ CHANGE 1: Dict keys filtering → SessionManager.get_meeting_participant_count()
    # OLD: session_key = get_session_key(meeting_id, user_id)
    # OLD: concurrent_sessions = [k for k in attendance_sessions.keys() if k.startswith(f"{meeting_id}_")]
    # OLD: logger.debug(f"MULTI-USER: Validation for {user_id}. {len(concurrent_sessions)} sessions active")
    concurrent_count = SessionManager.get_meeting_participant_count(meeting_id)
    logger.debug(f"MULTI-USER: Validation for {user_id}. {concurrent_count} sessions active")
...
def restore_session_from_db(meeting_id: str, user_id: str, session_key: str) -> bool:
    """
    ✅ FIXED: Helper function to restore session from database to Redis
    ✅ UPDATED: Uses SessionManager.create_session() instead of dict + lock
    
    REDIS MIGRATION CHANGES (2 total):
    1. with attendance_sessions_lock: attendance_sessions[session_key] = {...} → session_data = {...}
    2. (end of dict) → SessionManager.create_session(meeting_id, user_id, session_data)
    """
    try:
        # ✅ FIX: Try active session first
        db_session = AttendanceSession.objects.filter(
        ...
        # ✅ CHANGE 1: Remove lock, build session_data dict
        # OLD: with attendance_sessions_lock:
        # OLD:     attendance_sessions[session_key] = {
        session_data = {
            "meeting_id": meeting_id,
            "user_id": user_id,
            "session_active": db_session.session_active,
        ...
        # ✅ CHANGE 2: Dict assignment → SessionManager.create_session()
        # OLD: (closing brace was inside with-lock block)
...
def start_attendance_tracking(meeting_id: str, user_id, user_name: str = None) -> bool:
    ...
    REDIS MIGRATION CHANGES (7 total):
    1. attendance_sessions.keys() for concurrent count → SessionManager.get_meeting_participant_count()
    2. session_key in attendance_sessions → SessionManager.session_exists()
    3. attendance_sessions[session_key] = {...} (REJOIN) → SessionManager.create_session()
    4. attendance_sessions[session_key] = {...} (FIRST JOIN) → SessionManager.create_session()
    5. len([k for k in attendance_sessions.keys()...]) → SessionManager.get_meeting_participant_count()
    6. del attendance_sessions[session_key] on error → SessionManager.delete_session()
    7. All len(concurrent_sessions) refs → concurrent_count
    """
    user_id = str(user_id)
    session_key = get_session_key(meeting_id, user_id)
    
    # ✅ CHANGE 1: Dict keys → SessionManager.get_meeting_participant_count()
    # OLD: concurrent_sessions = [k for k in attendance_sessions.keys() if k.startswith(f"{meeting_id}_")]
    concurrent_count = SessionManager.get_meeting_participant_count(meeting_id)
    
    # ✅ CHANGE 2: Dict check → SessionManager.session_exists()
    # OLD: if session_key in attendance_sessions:
    if SessionManager.session_exists(meeting_id, user_id):
        logger.warning(f"MULTI-USER: Session already exists in Redis for {meeting_id}_{user_id}")
        return True
    ...
    try:
        ...
        identity_removal_count = existing_db.identity_removal_count
        behavior_removal_count = existing_db.behavior_removal_count
        continuous_violation_removal_count = existing_db.continuous_violation_removal_count
        ...
        # ============================================================
        # ✅ CHANGE 3: Dict assignment → SessionManager.create_session()
        # OLD: attendance_sessions[session_key] = { ... }
        # ============================================================
        session_data = {
            "meeting_id": meeting_id,
            ...
            "removal_reason": "",
            "continuous_violation_start_time": None,
            "continuous_violation_type": None,
            "last_detection_time": rejoin_last_detection_time,
            "detection_penalty_applied": extended_data.get('detection_penalty_applied', False),
            "warning_phase_complete": extended_data.get('warning_phase_complete', False),
            ...
            "session_started_at": current_time,
            "isolation_verified": True,
            # ✅ CHANGE 7a: len(concurrent_sessions) → concurrent_count
            "concurrent_participants_at_start": concurrent_count,
            
            # ==================== IDENTITY VERIFICATION STATE (PRESERVED/RESET) ====================
            "identity_warning_count": identity_current_cycle_warnings,
            "identity_consecutive_unknown_seconds": identity_consecutive_unknown,
            "identity_total_unknown_seconds": identity_total_unknown,
            "identity_is_removed": identity_is_removed,
            "identity_warnings": identity_warnings_list,
            "identity_last_check_time": current_time,
            "identity_can_rejoin": True,
            
            # ==================== REMOVAL TRACKING (PRESERVED) ====================
            "identity_removal_count": identity_removal_count,
            "identity_total_warnings": identity_total_warnings,
            "identity_current_cycle_warnings": identity_current_cycle_warnings,
            "behavior_removal_count": behavior_removal_count,
            "continuous_violation_removal_count": continuous_violation_removal_count,
            
            # ==================== BEHAVIOR MESSAGES (PRESERVED) ====================
        ...
        logger.info(
            ...
            f"  - Baseline Reset: Yes (will re-establish)\n"
            f"  - Detection Ready: Yes (last_detection_time set to past)\n"
            # ✅ CHANGE 7b: len(concurrent_sessions) + 1 → concurrent_count + 1
            f"  - Meeting has {concurrent_count + 1} participants"
        )
        ...
        # ============================================================
        # ✅ CHANGE 4: Dict assignment → SessionManager.create_session()
        # OLD: attendance_sessions[session_key] = { ... }
        # ============================================================
        session_data = {
            "meeting_id": meeting_id,
            ...
            "session_started_at": current_time,
            "isolation_verified": True,
            # ✅ CHANGE 7c: len(concurrent_sessions) → concurrent_count
            "concurrent_participants_at_start": concurrent_count,
            ...
            "identity_current_cycle_warnings": 0,
            "behavior_removal_count": 0,
            "continuous_violation_removal_count": 0,
            
            # ==================== BEHAVIOR MESSAGES (FRESH START) ====================
            "behavior_messages": {'warnings': [], 'detections': [], 'continuous_removals': []},
        }
        SessionManager.create_session(meeting_id, user_id, session_data)
        ...
        try:
            ...
            AttendanceSession.objects.update_or_create(
                ...
                defaults={
                    ...
                    'session_active': True,
                    'break_used': False,
                    'violations': json.dumps({'warnings': [], 'detections': [], 'continuous_removals': []}),
                    'session_start_time': timezone.now(),
                    'last_activity': timezone.now(),
                    'last_face_movement_time': current_time,
                    'inactivity_popup_shown': False,
            ...
            # ✅ CHANGE 5: Dict keys count → SessionManager.get_meeting_participant_count()
            # OLD: final_concurrent_count = len([k for k in attendance_sessions.keys() if k.startswith(f"{meeting_id}_")])
            final_concurrent_count = SessionManager.get_meeting_participant_count(meeting_id)
            ...
            logger.error(f"MULTI-USER: Failed to start tracking for {meeting_id}_{user_id}: {e}")
            logger.error(traceback.format_exc())
            # ✅ CHANGE 6: Dict delete → SessionManager.delete_session()
            # OLD: if session_key in attendance_sessions:
            # OLD:     del attendance_sessions[session_key]
...
def stop_attendance_tracking(meeting_id: str, user_id) -> bool:
    ...
    REDIS MIGRATION CHANGES (4 total):
    1. attendance_sessions.keys() filtering → SessionManager.get_meeting_participant_count()
    2. session_key in attendance_sessions + attendance_sessions[session_key] → SessionManager.get_session()
    3. del attendance_sessions[session_key] → SessionManager.delete_session()
    4. attendance_sessions.keys() for remaining count → SessionManager.get_meeting_participant_count()
    """
    user_id = str(user_id)
    session_key = get_session_key(meeting_id, user_id)
    
    # ✅ CHANGE 1: Dict keys filtering → SessionManager.get_meeting_participant_count()
    # OLD: other_participants = [k for k in attendance_sessions.keys() 
    #                           if k.startswith(f"{meeting_id}_") and k != session_key]
    # OLD: logger.info(f"MULTI-USER: Stopping tracking for {user_id}. {len(other_participants)} other participants unaffected")
    other_count = max(0, SessionManager.get_meeting_participant_count(meeting_id) - 1)
    logger.info(f"MULTI-USER: Stopping tracking for {user_id}. {other_count} other participants unaffected")
    
    # ✅ CHANGE 2: Dict check + access → SessionManager.get_session()
    # OLD: if session_key in attendance_sessions:
    ...
    if session is not None:
        ...
        try:
            ...
            if session.get('is_currently_on_break'):
                ...
                attendance_obj.current_break_start_time = None
                attendance_obj.save()
                # Save updated session to Redis so store_attendance_to_db reads latest state
        ...
        # ✅ CHANGE 3: Dict delete → SessionManager.delete_session()
        # OLD: del attendance_sessions[session_key]
        SessionManager.delete_session(meeting_id, user_id)
        
        # ✅ CHANGE 4: Dict keys filtering → SessionManager.get_meeting_participant_count()
        # OLD: remaining_participants = [k for k in attendance_sessions.keys() 
        #                               if k.startswith(f"{meeting_id}_")]
        # OLD: logger.info(f"MULTI-USER: User {user_id} stopped. {len(remaining_participants)} participants continue")
        remaining_count = SessionManager.get_meeting_participant_count(meeting_id)
        logger.info(f"MULTI-USER: User {user_id} stopped. {remaining_count} participants continue")
...
def store_attendance_to_db(meeting_id: str, user_id: str) -> bool:
    ...
    REDIS MIGRATION CHANGES (2 total):
    1. session_key not in attendance_sessions → SessionManager.get_session() returns None
    2. state = attendance_sessions[session_key] → state = SessionManager.get_session()
    """
    session_key = get_session_key(meeting_id, user_id)
    
    # ✅ CHANGE 1+2: Dict check + access → SessionManager.get_session()
    # OLD: if session_key not in attendance_sessions:
    # OLD:     logger.warning(f"⚠️ Cannot store attendance: Session not found for {meeting_id}_{user_id}")
    # OLD:     return False
    # OLD: state = attendance_sessions[session_key]
    state = SessionManager.get_session(meeting_id, user_id)
    if state is None:
        logger.warning(f"⚠️ Cannot store attendance: Session not found for {meeting_id}_{user_id}")
        return False
        ...
        with transaction.atomic():
            ...
            attendance_record, created = AttendanceSession.objects.update_or_create(
                ...
                defaults={
                    ...
                    'identity_total_unknown_seconds': state.get("identity_total_unknown_seconds", 0),
                    'identity_is_removed': state.get("identity_is_removed", False),
                    'identity_removal_time': timezone.now() if state.get("identity_is_removed", False) else None,
                    'identity_can_rejoin': state.get("identity_can_rejoin", True),
                    'identity_warnings': identity_warnings_json,
                    'identity_last_check_time': state.get("identity_last_check_time", 0.0),                    
                    # ==================== REMOVAL TRACKING FIELDS ====================
...
def store_all_active_sessions_to_db(meeting_id: str = None) -> dict:
    ...
    REDIS MIGRATION CHANGES (1 total):
    1. attendance_sessions.items() filtering → SessionManager.get_all_sessions()
    ...
    try:
        # ============================================================
        # STEP 1: Filter sessions based on meeting_id
        # ✅ CHANGE 1: Dict filtering → SessionManager.get_all_sessions()
        # OLD:
        # if meeting_id:
        #     sessions_to_store = [
        #         (session_key, session_data) 
        ...
        # NEW:
        # ============================================================
        sessions_list = SessionManager.get_all_sessions(meeting_id)
        sessions_to_store = [
            (f"{s.get('meeting_id', '')}_{s.get('user_id', '')}", s)
            for s in sessions_list
        ]
        logger.info(
            f"💾 BATCH STORAGE: Processing {len(sessions_to_store)} sessions"
            + (f" for meeting {meeting_id}" if meeting_id else " across ALL meetings")
        )
...
def auto_store_attendance_periodic(meeting_id: str, interval_seconds: int = 60, max_iterations: int = None) -> None:
    ...
    REDIS MIGRATION CHANGES (1 total):
    1. attendance_sessions.keys() filtering → SessionManager.get_meeting_participant_count()
       Also replaced len(active_sessions) → active_count throughout
    ...
    try:
        while True:
            ...
            # NEW:
            # ============================================================
            active_count = SessionManager.get_meeting_participant_count(meeting_id)
            ...
            try:
                logger.debug(
                    # ✅ CHANGED: len(active_sessions) → active_count
                    f"🔄 AUTO-BACKUP [{iteration}]: Starting backup for {active_count} sessions..."
                )
...
def verify_camera_resumed(request):
    ...
    REDIS MIGRATION CHANGES (4 total):
    1. session_key not in attendance_sessions → not SessionManager.session_exists()
    2. session = attendance_sessions[session_key] → SessionManager.get_session()
    3. Added SessionManager.save_session() before deadline-exceeded return
    4. Added SessionManager.save_session() after successful verification
    """
    try:
        data = json.loads(request.body)
        meeting_id = data.get('meeting_id')
        ...
        # ✅ CHANGE 1: Dict check → SessionManager.session_exists()
        # OLD: if session_key not in attendance_sessions:
        if not SessionManager.session_exists(meeting_id, user_id):
            logger.warning(f"⚠️ CAMERA VERIFY: Session not in Redis for {user_id}, attempting to restore from DB...")
        ...
        # ✅ CHANGE 2: Dict access → SessionManager.get_session()
        # OLD: session = attendance_sessions[session_key]
        session = SessionManager.get_session(meeting_id, user_id)
        if session is None:
            return JsonResponse({
        ...
        # Handle case where no verification is expected (unchanged)
        if not expected_token:
            logger.warning(f"CAMERA VERIFY: No token expected for {user_id}")
            if camera_active:
                return JsonResponse({
        ...
        if current_time > deadline:
            ...
            # ✅ CHANGE 3: Save modified session to Redis before returning
            SessionManager.save_session(meeting_id, user_id, session)
        ...
        # SUCCESS: Clear verification state (unchanged)
        session['camera_resume_expected'] = False
        session['camera_resume_deadline'] = None
        session['camera_confirmation_token'] = None
        session['camera_verified_at'] = current_time
        
        # ✅ CHANGE 4: Save modified session to Redis after successful verification
        SessionManager.save_session(meeting_id, user_id, session)
...
def pause_resume_attendance(request):
    ...
    ✅ FIXED: Enhanced pause/resume with STRICT 5-minute break enforcement
    ✅ FIXED: Correct handling of current_break_start_time (it's a float, not datetime)
    ✅ FIXED: Better state synchronization between Redis and database
    ✅ FIXED: Uses restore_session_from_db() instead of duplicated restoration code
    ✅ UPDATED: Uses Redis SessionManager instead of in-memory dict
    ✅ UPDATED: Preserves violation_start_times across breaks (never clears them)
    
    REDIS MIGRATION CHANGES (6 total):
    1. other_participants list comprehension → SessionManager.get_meeting_participant_count()
    2. session_key not in attendance_sessions → not SessionManager.session_exists()
    3. session = attendance_sessions[session_key] → SessionManager.get_session()
    4. Added SessionManager.save_session() before PAUSE return
    5. Added SessionManager.save_session() before RESUME return
    6. Added SessionManager.save_session() before every early/error return that modifies session
    ...
    try:
        ...
        # ✅ CHANGE 1: Dict keys list comprehension → SessionManager
        # OLD: other_participants = [k for k in attendance_sessions.keys() if k.startswith(f"{meeting_id}_") and k != session_key]
        # OLD: logger.info(f"MULTI-USER: {action} request for {user_id}. {len(other_participants)} other participants unaffected")
        other_count = max(0, SessionManager.get_meeting_participant_count(meeting_id) - 1)
        logger.info(f"MULTI-USER: {action} request for {user_id}. {other_count} other participants unaffected")
        ...
        # ============================================================================
        if not SessionManager.session_exists(meeting_id, user_id):
            logger.warning(f"⚠️ PAUSE_RESUME: Session not in Redis for {user_id}, attempting to restore from DB...")
        ...
        # ✅ CHANGE 3: Dict access → SessionManager.get_session()
        # OLD: session = attendance_sessions[session_key]
        session = SessionManager.get_session(meeting_id, user_id)
        if session is None:
            return JsonResponse({
        ...
        # ============================================================================
        # ✅ SYNC BREAK STATE FROM DATABASE TO REDIS (unchanged logic)
        # ============================================================================
        db_is_on_break = attendance_obj.is_currently_on_break or False
        memory_is_on_break = session.get('is_currently_on_break', False)
        ...
        if db_is_on_break != memory_is_on_break:
            logger.warning(
                f"⚠️ BREAK STATE MISMATCH for {user_id}:\n"
                f"   - DB is_on_break: {db_is_on_break}\n"
                f"   - Redis is_on_break: {memory_is_on_break}\n"
                f"   - Syncing Redis from DB..."
            )
            session['is_currently_on_break'] = db_is_on_break
        ...
        if action == 'pause':
            if session.get('is_currently_on_break', False):
                current_total_break_time = calculate_current_break_time(session, current_time)
                logger.warning(f"⚠️ PAUSE REJECTED: {user_id} is already on break")
                # ✅ CHANGE 4a: Save synced state to Redis before returning
                SessionManager.save_session(meeting_id, user_id, session)
                return JsonResponse({
                    'success': False,
                    'error': 'Already on break',
            ...
            if current_total_break_time >= max_break_time:
                ...
                # ✅ CHANGE 4b: Save synced state to Redis before returning
                SessionManager.save_session(meeting_id, user_id, session)
                return JsonResponse({
                    'success': False,
                    'error': 'Break time limit exceeded - No more break time available',
            ...
            if break_duration_available <= 0:
                ...
                # ✅ CHANGE 4c: Save synced state to Redis before returning
                SessionManager.save_session(meeting_id, user_id, session)
                return JsonResponse({
                    'success': False,
                    'error': 'No break time remaining',
            ...
            # ✅ ALLOW BREAK: Update session state (unchanged)
            session['is_currently_on_break'] = True
            session['current_break_start_time'] = current_time
            session['session_active'] = False
            session['break_count'] = session.get('break_count', 0) + 1
            ...
            # ✅ CHANGE 5: Save modified session to Redis before returning
            SessionManager.save_session(meeting_id, user_id, session)
        ...
        # ============================================================================
            ...
            # ✅ CASE 1: Neither DB nor Redis says on break
            if not db_is_on_break and not memory_is_on_break:
                logger.warning(f"⚠️ RESUME REJECTED: {user_id} is NOT on break (both DB and Redis)")
                # ✅ CHANGE 6a: Save synced state to Redis before returning
                SessionManager.save_session(meeting_id, user_id, session)
                return JsonResponse({
                    'success': False,
                    'error': 'Not currently on break',
            ...
            # ✅ CASE 2: DB says on break but Redis doesn't - sync from DB
            if db_is_on_break and not memory_is_on_break:
                logger.warning(f"⚠️ STATE MISMATCH: Syncing break state from DB for {user_id}")
                session['is_currently_on_break'] = True
                ...
                if attendance_obj.current_break_start_time:
                    if isinstance(attendance_obj.current_break_start_time, (int, float)):
                        ...
                        try:
                            ...
                            session['current_break_start_time'] = current_time - 60
                else:
                    session['current_break_start_time'] = current_time - 60
            
            # ✅ CASE 3: Redis says on break but DB doesn't - trust DB
            if memory_is_on_break and not db_is_on_break:
                logger.warning(f"⚠️ STATE MISMATCH: Redis says on break but DB doesn't for {user_id}")
                session['is_currently_on_break'] = False
                session['current_break_start_time'] = None
                # ✅ CHANGE 6b: Save corrected state to Redis before returning
                SessionManager.save_session(meeting_id, user_id, session)
                return JsonResponse({
                    'success': False,
                    'error': 'Not currently on break',
            ...
            # ✅ NOW PROCEED WITH RESUME (all logic below unchanged)
            break_start = session.get('current_break_start_time')
            if break_start:
                break_duration_used = current_time - break_start
            else:
                break_duration_used = 60
                logger.warning(f"⚠️ No break_start_time found, using default {break_duration_used}s")
            
            update_break_time_used(session, attendance_obj, current_time)
            
            # ✅ APPLY BREAK PENALTY (unchanged)
            penalty_per_5_minutes = AttendanceConfig.BREAK_PENALTY
            break_penalty = (break_duration_used / 300.0) * penalty_per_5_minutes
            session['attendance_penalty'] = session.get('attendance_penalty', 0) + break_penalty
            ...
            # ✅ CHECK: Auto-set break_used if time exhausted (unchanged)
            total_break_used = session.get('total_break_time_used', 0)
            if total_break_used >= max_break_time:
                session['break_used'] = True
                logger.info(f"🚫 break_used AUTO-SET to True for {user_id} - Time exhausted ({total_break_used}s >= {max_break_time}s)")
            
            # ✅ UPDATE SESSION STATE (unchanged)
            session['is_currently_on_break'] = False
            session['current_break_start_time'] = None
            session['session_active'] = True
            session['last_face_movement_time'] = current_time
            ...
            # ✅ CRITICAL: DO NOT CLEAR violation_start_times (unchanged)
            logger.info(
                f"🔒 VIOLATION TIMES PRESERVED for {user_id} after break: "
                f"{len(session.get('violation_start_times', {}))} violations kept"
            )
            
            # ✅ ACTIVATE GRACE PERIOD (unchanged)
            grace_period_until = current_time + AttendanceConfig.GRACE_PERIOD_DURATION
            session['grace_period_active'] = True
            session['grace_period_until'] = grace_period_until
            
            # ✅ CAMERA VERIFICATION (unchanged)
            verification_token = generate_camera_verification_token(meeting_id, user_id, current_time)
            verification_deadline = current_time + AttendanceConfig.CAMERA_VERIFICATION_TIMEOUT
            ...
            # ✅ UPDATE EXTENDED DATA (unchanged)
            try:
                extended_data = get_extended_tracking_data(attendance_obj)
                extended_data['camera_resume_expected'] = True
                extended_data['camera_resume_deadline'] = verification_deadline
            ...
            # ✅ CHANGE 6c: Save fully modified session to Redis before returning
            SessionManager.save_session(meeting_id, user_id, session)
...
def detect_violations(request):
    """
    ✅ UPDATED: Complete detect_violations with Redis SessionManager
    ✅ UPDATED: Uses mediapipe_pool instead of per-request instantiation
    ✅ UPDATED: Frame throttle (3 second minimum interval - matches frontend)
    ✅ UPDATED: Routine per-frame DB writes removed (periodic_saver handles it)
    ✅ NEW: Network drop detection - prevents unfair penalties during WiFi issues
    ✅ PRESERVED: All existing features (identity, grace, camera, break, removal, messages)
    
    CHANGES FROM ORIGINAL:
    1. attendance_sessions[session_key] → SessionManager.get_session()
    2. get_mediapipe_results_per_request() → get_mediapipe_results()
    3. Added 3-second frame throttle to reduce CPU/Redis load
    4. Removed end-of-frame DB save block (periodic_saver runs every 30s)
    5. Added SessionManager.save_session() before final return
    6. KEPT: All DB writes for warnings, detections, penalties, removals (critical events)
    7. NEW: Network drop detection after frame throttle (10s threshold, 5s grace period)
    """
    try:
        data = json.loads(request.body)
        meeting_id = data.get('meeting_id')
        ...
        # ============================================================================
        # ✅ SESSION LOOKUP FROM REDIS (replaces attendance_sessions dict)
        # ============================================================================
        if not SessionManager.session_exists(meeting_id, user_id):
            logger.warning(f"⚠️ DETECT: Session not in Redis for {user_id}, attempting to restore from DB...")
            
            if not restore_session_from_db(meeting_id, user_id, get_session_key(meeting_id, user_id)):
                # No existing session in DB - create new one
                logger.info(f"MULTI-USER: No existing session in DB, starting new session for {user_id}")
                start_success = start_attendance_tracking(meeting_id, user_id)
                if not start_success:
        ...
        # ============================================================
        # ✅ FRAME THROTTLE (3 second minimum interval)
        # Matches frontend DETECTION_INTERVAL of 3000ms
        # Reduces CPU, Redis, and MediaPipe load
        # ============================================================
        MINIMUM_FRAME_INTERVAL = 3.0
        current_time_throttle = time.time()
        last_frame_time = session.get('last_frame_processed', 0)
        if (current_time_throttle - last_frame_time) < MINIMUM_FRAME_INTERVAL:
            return JsonResponse({
                "status": "throttled",
                "message": "Frame too soon, skipped",
                "retry_after": round(MINIMUM_FRAME_INTERVAL - (current_time_throttle - last_frame_time), 2),
                "attendance_percentage": max(0, 100 - session.get("attendance_penalty", 0)),
            })
        session['last_frame_processed'] = current_time_throttle

        # ============================================================
        # ✅ NEW: NETWORK DROP DETECTION
        # ============================================================
        # If no frame received for 10+ seconds, assume network issue.
        # Normal frame interval is 3 seconds, so 10 seconds = 3+ missed frames.
        # This CANNOT be normal behavior — it's definitely a network problem.
        #
        # Without this: WiFi drop → backend thinks "face not visible" / "inactivity"
        #   → unfair warnings, penalties, and even removal after 2 minutes
        #
        # With this: WiFi drop → backend detects gap → resets all timers
        #   → grants 5-second grace period → user gets clean restart
        #
        # THRESHOLDS:
        #   NETWORK_DROP_THRESHOLD = 10.0s (3+ missed frames = network issue)
        #   NETWORK_GRACE_PERIOD = 5.0s (allow camera/face to stabilize after reconnect)
        # ============================================================
        NETWORK_DROP_THRESHOLD = 10.0
        NETWORK_GRACE_PERIOD = 5.0
        time_since_last_frame = current_time_throttle - last_frame_time if last_frame_time > 0 else 0

        if time_since_last_frame > NETWORK_DROP_THRESHOLD and last_frame_time > 0:
            logger.warning(
                f"🌐 NETWORK DROP DETECTED for {user_id}:\n"
                f"   No frame for {time_since_last_frame:.1f}s (threshold: {NETWORK_DROP_THRESHOLD}s)\n"
                f"   Last frame at: {last_frame_time:.1f}\n"
                f"   Current time: {current_time_throttle:.1f}\n"
                f"   Action: Resetting violation timers, granting {NETWORK_GRACE_PERIOD}s grace period"
            )

            # ✅ RESET 1: Don't count this gap as inactivity
            # Without this: time_since_movement = 30s → "Inactivity detected" violation
            session['last_face_movement_time'] = current_time_throttle
            session['inactivity_popup_shown'] = False

            # ✅ RESET 2: Don't count toward continuous 2-minute removal timer
            # Without this: 2-min timer keeps running during WiFi drop → unfair removal
            if session.get('continuous_violation_start_time') is not None:
                logger.info(
                    f"🌐 NETWORK: Resetting 2-minute removal timer for {user_id} "
                    f"(was running since {session['continuous_violation_start_time']:.1f})"
                )
                session['continuous_violation_start_time'] = None

            # ✅ RESET 3: Clear current 20-second violation tracking
            # Without this: violation timer includes WiFi gap → instant warning on reconnect
            session['violation_continuous_timer'] = None
            session['violation_current_type'] = None
            session['violation_popup_shown'] = False

            # ✅ GRANT: Grace period after reconnect (5 seconds)
            # Allows camera to re-initialize and face to be detected before checking violations
            session['grace_period_active'] = True
            session['grace_period_until'] = current_time_throttle + NETWORK_GRACE_PERIOD

            # ✅ Save session and return grace period response immediately
            # No violation processing this frame — just acknowledge reconnection
            SessionManager.save_session(meeting_id, user_id, session)
            ...
            return JsonResponse({
                ...
                "attendance_percentage": max(0, 100 - session.get("attendance_penalty", 0)),
                "grace_period_active": True,
                "grace_period_expires_in": NETWORK_GRACE_PERIOD,
                "network_reconnected": True,
                "message": "Network reconnected - grace period active",
                "frame_count": session.get("frame_processing_count", 0),
                "baseline_established": session.get("baseline_established", False),
                "face_detected": False,
                "warning_count": session.get("warning_count", 0),
                "popup_count": session.get("popup_count", 0),
                "max_warnings": 4,
                "detection_counts": session.get("detection_counts", 0),
                "warning_phase_complete": session.get("warning_phase_complete", False),
                "in_warning_phase": session.get("popup_count", 0) < 4,
                "in_detection_phase": session.get("warning_phase_complete", False),
                "continuous_violation_duration": 0,
                "time_until_removal": 0,
                "continuous_violation_active": False,
        ...
        # ============================================================
        # ✅ DECODE FRAME ONCE (used for both identity AND behavior)
        # ============================================================
        frame = decode_image(frame_data)
        if frame is None:
            return JsonResponse({"status": "error", "message": "Failed to decode frame"}, status=400)
        ...
        if violation_to_show is not None and (current_time - session.get("last_popup_time", 0)) >= 20:
            ...
            if session["popup_count"] < 4:
                ...
                # ✅ KEPT: DB write for warnings (critical event)
                try:
                    db_session = AttendanceSession.objects.get(meeting_id=meeting_id, user_id=user_id)
                    extended_data = get_extended_tracking_data(db_session)
                    extended_data['warning_count'] = session["popup_count"]
                ...
                # ✅ KEPT: Save warning message (critical event)
                try:
                    db_session = AttendanceSession.objects.get(meeting_id=meeting_id, user_id=user_id)
                    db_session.add_behavior_warning(
                        warning_number=session["popup_count"],
                ...
                if time_since_last_detection >= 20:
                    ...
                    # ✅ KEPT: Save detection (critical event)
                    try:
                        db_session = AttendanceSession.objects.get(meeting_id=meeting_id, user_id=user_id)
                        extended_data = get_extended_tracking_data(db_session)
                        extended_data['detection_counts'] = session["detection_counts"]
                    ...
                    if new_penalty > 0:
                        ...
                        # ✅ KEPT: Save penalty (critical event)
                        try:
                            db_session = AttendanceSession.objects.get(meeting_id=meeting_id, user_id=user_id)
                            db_session.attendance_penalty = session["attendance_penalty"]
                            db_session.attendance_percentage = max(0, 100 - session["attendance_penalty"])
                    ...
                    # ✅ KEPT: Save detection message (critical event)
                    try:
                        db_session = AttendanceSession.objects.get(meeting_id=meeting_id, user_id=user_id)
                        db_session.add_behavior_detection(
                            detection_number=session["detection_counts"],
        ...
        if unique_violations:
            ...
            if session.get("warning_phase_complete", False):
                ...
                if continuous_duration >= 120:
                    ...
                    # ✅ KEPT: Save removal (critical event)
                    try:
                        db_session = AttendanceSession.objects.get(meeting_id=meeting_id, user_id=user_id)
                        db_session.behavior_removal_count = session["behavior_removal_count"]
                        db_session.continuous_violation_removal_count = session["continuous_violation_removal_count"]
        ...
        # ============================================================
        # ✅ REMOVED: Routine per-frame DB save block
        # The periodic_saver.py (every 30s) handles bulk DB writes.
        # Critical events (warnings, detections, penalties, removals)
        # are STILL saved immediately in the sections above.
        # This eliminates ~250 DB writes/sec → ~2 DB writes/sec.
        # ============================================================
        
        # Calculate continuous duration for response
        continuous_duration = 0
        if session.get("continuous_violation_start_time") and session.get("warning_phase_complete", False):
            continuous_duration = current_time - session["continuous_violation_start_time"]
        
        # ============================================================
        # ✅ SAVE SESSION TO REDIS (replaces per-frame DB write)
        # ============================================================
        SessionManager.save_session(meeting_id, user_id, session)
...
def take_break(request):
    ...
    2. session_key not in attendance_sessions → not SessionManager.session_exists()
    3. session = attendance_sessions[session_key] → SessionManager.get_session()
    4. Added SessionManager.save_session() before return after modifying session
    5. resume_after_break inner function: session_key in attendance_sessions → SessionManager.get_session()
    6. resume_after_break inner function: Added SessionManager.save_session() after modifications
    """
    try:
        data = json.loads(request.body)
        meeting_id = data.get("meeting_id")
        ...
        # OLD: if session_key not in attendance_sessions:
        if not SessionManager.session_exists(meeting_id, user_id):
            logger.warning(f"⚠️ BREAK: Session not in Redis for {user_id}, attempting to restore from DB...")
        ...
        # ✅ CHANGE 4: Save modified session to Redis
        SessionManager.save_session(meeting_id, user_id, session)
        ...
        def resume_after_break():
            ...
            # ✅ CHANGE 5: Dict check → SessionManager.get_session()
            # OLD: if session_key in attendance_sessions:
            #          local_session = attendance_sessions[session_key]
            local_session = SessionManager.get_session(meeting_id, user_id)
            if local_session is not None:
                ...
                # Setup camera verification (unchanged)
                verification_token = generate_camera_verification_token(meeting_id, user_id, current_time_local)
                verification_deadline = current_time_local + AttendanceConfig.CAMERA_VERIFICATION_TIMEOUT
                local_session["camera_resume_expected"] = True
                local_session["camera_resume_deadline"] = verification_deadline
                ...
                # ✅ CHANGE 6: Save modified session to Redis after all changes
                SessionManager.save_session(meeting_id, user_id, local_session)
...
def get_attendance_status(request):
    ...
    REDIS MIGRATION CHANGES (3 total):
    1. attendance_sessions.keys() filtering → SessionManager.get_meeting_participant_count()
    2. session_key in attendance_sessions + attendance_sessions[session_key] → SessionManager.get_session()
    3. len(concurrent_sessions) → concurrent_count (in all 3 response cases)
    ...
    try:
        ...
        # ✅ CHANGE 1: Dict keys filtering → SessionManager.get_meeting_participant_count()
        # OLD: concurrent_sessions = [k for k in attendance_sessions.keys() if k.startswith(f"{meeting_id}_")]
        # OLD: other_participants_count = len([k for k in concurrent_sessions if k != session_key])
        concurrent_count = SessionManager.get_meeting_participant_count(meeting_id)
        other_participants_count = max(0, concurrent_count - 1)
        
        # ============================================================
        # CASE 1: ACTIVE SESSION IN REDIS
        # ✅ CHANGE 2: Dict check + access → SessionManager.get_session()
        # OLD: if session_key in attendance_sessions:
        # OLD:     session = attendance_sessions[session_key]
        # ============================================================
        ...
        if session is not None:
            ...
            return JsonResponse({
                ...
                # ==================== MULTI-USER INFO ====================
                "user_isolation_verified": True,
                # ✅ CHANGE 3a: len(concurrent_sessions) → concurrent_count
                "concurrent_participants": concurrent_count,
                "other_participants_count": other_participants_count,
            })
        
        # ============================================================
        # CASE 2: SESSION IN DATABASE (NOT IN REDIS)
        # ============================================================
        try:
            attendance_obj = AttendanceSession.objects.get(meeting_id=meeting_id, user_id=user_id)
            break_time_remaining = max(0, (attendance_obj.max_break_time_allowed or 300) - (attendance_obj.total_break_time_used or 0))
            ...
            return JsonResponse({
                ...
                # ==================== MULTI-USER INFO ====================
                "user_isolation_verified": True,
                # ✅ CHANGE 3b: len(concurrent_sessions) → concurrent_count
                "concurrent_participants": concurrent_count,
                "other_participants_count": other_participants_count,
            })
            ...
            return JsonResponse({
                ...
                # ==================== MULTI-USER INFO ====================
                "user_isolation_verified": True,
                # ✅ CHANGE 3c: len(concurrent_sessions) → concurrent_count
                "concurrent_participants": concurrent_count,
                "other_participants_count": other_participants_count,
            })
...
def start_attendance_tracking_api(request):
    ...
    REDIS MIGRATION CHANGES (2 total):
    1. attendance_sessions.keys() filtering before start → SessionManager.get_meeting_participant_count()
    2. attendance_sessions.keys() filtering after start → SessionManager.get_meeting_participant_count()
    """
    try:
        data = json.loads(request.body)
        meeting_id = data.get('meeting_id')
        ...
        # ✅ CHANGE 1: Dict keys filtering → SessionManager.get_meeting_participant_count()
        # OLD: concurrent_sessions = [k for k in attendance_sessions.keys() if k.startswith(f"{meeting_id}_")]
        concurrent_before = SessionManager.get_meeting_participant_count(meeting_id)
        
        success = start_attendance_tracking(meeting_id, user_id_str, user_name)
        
        if success:
            # ✅ CHANGE 2: Dict keys filtering → SessionManager.get_meeting_participant_count()
            # OLD: final_concurrent_sessions = [k for k in attendance_sessions.keys() if k.startswith(f"{meeting_id}_")]
            concurrent_after = SessionManager.get_meeting_participant_count(meeting_id)
            ...
            return JsonResponse({
                ...
                'camera_verification_timeout_seconds': AttendanceConfig.CAMERA_VERIFICATION_TIMEOUT,
                'user_isolation_verified': True,
                # ✅ CHANGED: len(concurrent_sessions) → concurrent_before
                'concurrent_participants_before': concurrent_before,
                # ✅ CHANGED: len(final_concurrent_sessions) → concurrent_after
                'concurrent_participants_after': concurrent_after,
            })
        else:
            return JsonResponse({'success': False, 'error': 'Failed to start tracking'}, status=500)
...
def stop_attendance_tracking_api(request):
    ...
    REDIS MIGRATION CHANGES (3 total):
    1. attendance_sessions.keys() before stop → SessionManager.get_meeting_participant_count()
    2. attendance_sessions.keys() after stop → SessionManager.get_meeting_participant_count()
    3. len(concurrent_sessions_after) == 0 → concurrent_after == 0
    """
    try:
        data = json.loads(request.body)
        meeting_id = data.get('meeting_id')
        ...
        # ✅ CHANGE 1: Dict keys filtering → SessionManager.get_meeting_participant_count()
        # OLD: concurrent_sessions_before = [k for k in attendance_sessions.keys() if k.startswith(f"{meeting_id}_")]
        # OLD: session_key = get_session_key(meeting_id, user_id_str)
        # OLD: other_participants_before = [k for k in concurrent_sessions_before if k != session_key]
        concurrent_before = SessionManager.get_meeting_participant_count(meeting_id)
        other_before = max(0, concurrent_before - 1)
        
        # ✅ FIRST: Stop the attendance session (stops frame processing)
        success = stop_attendance_tracking(meeting_id, user_id_str)
        
        # ✅ CHANGE 2: Dict keys filtering → SessionManager.get_meeting_participant_count()
        # OLD: concurrent_sessions_after = [k for k in attendance_sessions.keys() if k.startswith(f"{meeting_id}_")]
        concurrent_after = SessionManager.get_meeting_participant_count(meeting_id)
        
        # ✅ CHANGE 3: len() == 0 → count == 0
        # OLD: is_last_participant = len(concurrent_sessions_after) == 0
        is_last_participant = concurrent_after == 0
        gpu_released = False
        ...
        if success and is_last_participant:
            ...
            if not is_last_participant:
                # ✅ CHANGED: len(concurrent_sessions_after) → concurrent_after
                logger.info(f"ℹ️ Meeting {meeting_id} still has {concurrent_after} active participants. GPU memory retained.")
        ...
        return JsonResponse({
            ...
            'timestamp': timezone.now().isoformat(),
            'user_isolation_verified': True,
            # ✅ CHANGED: len(concurrent_sessions_before) → concurrent_before
            'concurrent_participants_before': concurrent_before,
            # ✅ CHANGED: len(concurrent_sessions_after) → concurrent_after
            'concurrent_participants_after': concurrent_after,
            # ✅ CHANGED: len(other_participants_before) → other_before
            'other_participants_unaffected': other_before,
            'is_last_participant': is_last_participant,
            'gpu_memory_released': gpu_released,
        })

...
"""
Shared Face Recognition Model Module — SERVICE 1 (CPU ONLY)
=============================================================
Used by face_embeddings.py (Registration Service) ONLY.

Hardware: CPU ONLY (GPU reserved for Service 2 — Identity Verification)
When Used: During user registration / photo update (ONE TIME per user)
What it Does: Takes user photo → generates 512-dim face embedding → stores in MongoDB

Author: Face Recognition System
Version: 2.0.0 — Microservice Split (CPU Only)
"""

import os

# --- CPU ONLY Configuration ---
# Service 1 (Embeddings/Registration) runs on CPU only
# Reason: Runs ONCE per user during registration — GPU not needed
# GPU is reserved for Service 2 (Identity Verification) which runs continuously
os.environ.update({
    "CUDA_VISIBLE_DEVICES": "-1",       # DISABLE GPU (force CPU only)
    "OMP_NUM_THREADS": "4",
})
...
class SharedFaceModel:
    ...
    def _initialize_model(self):
        """Initialize InsightFace model once"""
        try:
            logger.info(f"ðŸ”¹ Initializing Shared InsightFace Model: {FACE_MODEL_NAME}")
            logger.info(f"   Detection Size: {FACE_DETECTION_SIZE}")
            ...
            self._app = FaceAnalysis(
                ...
                providers=['CPUExecutionProvider']  # CPU ONLY — GPU reserved for Service 2
            )
    ...
    def unload_model(self):
        ...
        try:
            if self._app is not None:
                logger.info("ðŸ”¹ Unloading InsightFace model and releasing GPU memory...")
            ...
            logger.error(f"âŒ Error unloading model: {e}")
    ...
    def extract_embedding(self, image_data, return_face_info=False):
        ...
        try:
            ...
            logger.error(f"âŒ Error extracting embedding: {e}", exc_info=True)
            raise ValueError(f"Failed to process image: {str(e)}")
    ...
    def _convert_to_numpy(self, image_data):
        ...
        try:
            ...
            logger.error(f"âŒ Error converting image to numpy: {e}")
            raise ValueError(f"Failed to convert image: {str(e)}")
    ...
    def detect_face(self, image_data):
        ...
        try:
            ...
            logger.error(f"âŒ Error detecting face: {e}")
            return {
                'detected': False,
                'face_count': 0,
                'largest_face_bbox': None,
    ...
    def compare_embeddings(self, embedding1, embedding2, method='cosine'):
        ...
        try:
            ...
            logger.error(f"âŒ Error comparing embeddings: {e}")
            raise
...
def unload_face_model():
    ...
    if _global_face_model is not None:
        _global_face_model.unload_model()
        _global_face_model = None
        logger.info("ðŸ”„ Global face model instance reset")
# ============================================================================
# CONVENIENCE FUNCTIONS
# ============================================================================
...
# except Exception as e:
#     logger.error(f"âŒ Failed to initialize shared face model: {e}")
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("ðŸ§ª Testing Shared Face Model")
    print("=" * 80)
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            # Test embedding extraction
            print("\nðŸ§® Testing embedding extraction...")
            face_info = model.extract_embedding(image_data, return_face_info=True)
            print(f"   Embedding dimension: {len(face_info['embedding'])}")
            print(f"   Detection score: {face_info['det_score']:.4f}")
            print(f"   Age: {face_info['age']}")

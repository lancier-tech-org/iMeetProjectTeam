...
@csrf_exempt
def validate_face_endpoint(request):
    ...
    try:
        ...
        if not photo_base64:
            return JsonResponse({'valid': False, 'error': 'No photo provided'}, status=400)
        from embeddings.face_embeddings import validate_human_face
        result = validate_human_face(photo_base64)
        return JsonResponse(result)
    except Exception as e:
        return JsonResponse({'valid': False, 'error': str(e)}, status=500)

@csrf_exempt
def process_embedding_endpoint(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'POST required'}, status=405)
    try:
        import json
        ...
        if not user_id or not photo_id:
            return JsonResponse({'error': 'user_id and photo_id required'}, status=400)
        from embeddings.face_embeddings import process_profile_photo_embedding
        embedding_id = process_profile_photo_embedding(user_id, photo_id)
        if embedding_id:
            return JsonResponse({'embedding_id': embedding_id}, status=200)

                    ...
                    try:
                        ...
                        if is_recording_enabled:
                            ...
                            if active_hosts_count == 0:
                                ...
                                try:
                                    from clients.recording_client import stream_recording_service
        ...
        try:
            ...
            for lk_participant in livekit_participants:
                try:
                    ...
                    if user_id:
                        # Extract join timestamp from identity (format: user_{id}_{timestamp}_{random})
                        identity_timestamp = 0
                        try:
                            id_parts = identity.split('_')
                            if len(id_parts) >= 3 and id_parts[2].isdigit():
                                identity_timestamp = int(id_parts[2])
                        except Exception:
                            pass
                        
                        user_key = str(user_id)
                        existing_entry = livekit_user_mapping.get(user_key)
                        
                        # Only keep the LATEST identity per user (highest timestamp = most recent connection)
                        if not existing_entry or identity_timestamp > existing_entry.get('identity_timestamp', 0):
                            livekit_user_mapping[user_key] = {
                                **lk_participant,
                                'parsed_user_id': user_id,
        ...
        try:
            with connection.cursor() as cursor:
                ...
                for row in db_participants:
                    ...
                    # Parse leave_times safely
                    leave_times = []
                    try:
                        if leave_times_json is None:
                            leave_times = []
                        elif isinstance(leave_times_json, str):
                            leave_times = json.loads(leave_times_json) if leave_times_json.strip() else []
                        elif isinstance(leave_times_json, list):
                            leave_times = leave_times_json
                        else:
                            leave_times = []
                    except Exception as e:
                        logging.error(f"[SYNC-FIXED] Error parsing leave_times for user {user_id}: {e}")
                        leave_times = []

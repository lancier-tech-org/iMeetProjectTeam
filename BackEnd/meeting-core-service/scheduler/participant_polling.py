...
This polling task:
1. Detects disconnected participants (compares LiveKit vs Database)
2. Uses a 30-second grace period before marking as left (prevents rapid join/leave)
3. Stores leave time in Leave_Times JSON array
4. Marks Is_Currently_Active = FALSE
5. SELF-HEALS users who are in LiveKit but wrongly marked inactive in DB
"""
...
def _extract_user_id_from_identity(livekit_identity):
    """
    Robust user_id extraction from LiveKit identity.
    Same logic as Sync_LiveKit_Participants_Fixed in participants.py.
    Returns user_id as string, or None if nothing matches.
    """
    if not livekit_identity:
        return None

    # Method 1: pattern "user_<id>_..." (case-insensitive)
    if 'user_' in livekit_identity.lower():
        parts = livekit_identity.split('_')
        if len(parts) >= 2 and parts[1].isdigit():
            return parts[1]

    # Method 2: direct numeric identity
    if livekit_identity.isdigit():
        return livekit_identity

    # Method 3: regex — first number anywhere in identity
    numbers = re.findall(r'\d+', livekit_identity)
    if numbers:
        return numbers[0]
...
def sync_participants_polling():
    ...
    1. Gets all active meetings
    2. For each meeting: compares LiveKit participants with database
    3. SELF-HEAL: Reactivates users in LiveKit but marked inactive in DB
    4. If user missing from LiveKit: start grace period timer
    5. If user still missing after 30 seconds: mark as left
    6. If user reappears: cancel grace period
    """
    from meetings.meetings import livekit_service
        ...
        with connection.cursor() as cursor:
            ...
            for meeting_id, room_name, meeting_status in active_meetings:
                try:
                    ...
                    # Build set of user_ids currently in LiveKit (using robust extraction)
                    livekit_user_ids = set()
                    for lk_identity in livekit_identities:
                        extracted = _extract_user_id_from_identity(lk_identity)
                        if extracted:
                            livekit_user_ids.add(extracted)
                    ...
                    # ===== B.5: SELF-HEAL — REACTIVATE USERS IN LIVEKIT BUT MARKED INACTIVE =====
                    # This is the KEY fix: if LiveKit has someone but DB says they left,
                    # the DB is wrong. Reactivate them so co-host assign / remove works.
                    active_user_ids_in_db = {str(row[1]) for row in db_participants}
                    stuck_user_ids = livekit_user_ids - active_user_ids_in_db

                    for stuck_user_id in stuck_user_ids:
                        cursor.execute("""
                            SELECT ID, Leave_Times 
                            FROM tbl_Participants
                            WHERE Meeting_ID = %s AND User_ID = %s AND Is_Currently_Active = FALSE
                            ORDER BY occurrence_number DESC, ID DESC
                            LIMIT 1
                        """, [meeting_id, stuck_user_id])
                        
                        stuck_row = cursor.fetchone()
                        if not stuck_row:
                            continue
                        
                        stuck_participant_id, stuck_leave_times_json = stuck_row
                        
                        # Remove phantom last leave_time (user is actually still connected)
                        try:
                            stuck_leave_times = json.loads(stuck_leave_times_json) if isinstance(stuck_leave_times_json, str) else (stuck_leave_times_json or [])
                        except Exception:
                            stuck_leave_times = []
                        
                        if stuck_leave_times:
                            stuck_leave_times.pop()
                        ...
                        cursor.execute("""
                            ...
                            SET Is_Currently_Active = TRUE,
                                Leave_Times = %s
                            WHERE ID = %s
                        """, [json.dumps(stuck_leave_times), stuck_participant_id])
                        
                        # Clear any grace period tracking for this user
                        grace_key = f"{meeting_id}:{stuck_user_id}"
                        if grace_key in _missing_since:
                            del _missing_since[grace_key]
                        
                        sync_results['participants_self_healed'] += 1
                        logger.info(f"🔄 [POLLING] Self-healed user {stuck_user_id} in meeting {meeting_id} — was marked inactive but still in LiveKit")

                    # Re-fetch db_participants if we reactivated anyone
                    if stuck_user_ids:
                        cursor.execute("""
                            SELECT ID, User_ID, occurrence_number, Role, Leave_Times, Is_Currently_Active, Join_Times
                            FROM tbl_Participants 
                    ...
                    for participant_row in db_participants:
                        ...
                        # Robust check: is this user still in LiveKit?
                        is_still_in_livekit = str(user_id) in livekit_user_ids
                        ...
                        if is_still_in_livekit:
                            ...
                            # Skip users who joined very recently — LiveKit may not have registered them yet
                            skip_freshly_joined = False
                            try:
                                _join_times = json.loads(join_times_json) if isinstance(join_times_json, str) else (join_times_json or [])
                                if _join_times:
...
✅ [POLLING] Sync cycle completed:
   ...
   - Participants checked: {sync_results['participants_checked']}
   - Marked as left: {sync_results['participants_marked_left']}
   - Self-healed: {sync_results['participants_self_healed']}
   - Errors: {len(sync_results['errors'])}
        """)

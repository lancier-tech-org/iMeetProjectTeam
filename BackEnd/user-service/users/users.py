...
# ============================================================================
# ADD THIS IMPORT - Login History Module
# ============================================================================
# Login history - not available in user-service (lives in meeting-core-service)
LOGIN_HISTORY_ENABLED = False
def record_login_history(*args, **kwargs): return None
def record_logout(*args, **kwargs): return False
def create_login_history_table(): pass
# Global Variables
TBL_USER = 'tbl_Users'
...
# ---------------------------------------------------------------------
# 📧 Email Configuration
# Primary: AWS SES via Django settings (noreply@lancieretech.com)
# Fallback: Gmail SMTP via settings.EMAIL_FALLBACK (prathigudupuj@gmail.com)
# All values loaded from settings.py which reads from environment variables
# ---------------------------------------------------------------------
...
# ============================================================================
try:
    from clients.face_auth_client import (
    ...
    logging.info("✓ Face recognition client loaded successfully")
except ImportError as e:
    logging.warning(f"⚠ Face recognition client not available: {e}")
...
# ==============================================================================
# STEP 1: ADD THIS VALIDATION DICTIONARY (after your imports)
# ==============================================================================
...
PHONE_VALIDATION_RULES: Dict[str, Dict[str, Any]] = {
    '+1': {
        'name': 'United States',
        'length': [10],
        'regex': r'^[2-9]\d{2}[2-9]\d{6}$',
        'error': 'US number must be 10 digits. Area code cannot start with 0 or 1. Example: 2025551234'
    },
    '+91': {
        'name': 'India',
        'length': [10],
        'regex': r'^[6-9]\d{9}$',
        'error': 'Indian mobile must be 10 digits starting with 6, 7, 8, or 9. Example: 9876543210'
    },
    '+44': {
        'name': 'United Kingdom',
        'length': [10],
        'regex': r'^7\d{9}$',
        'error': 'UK mobile must be 10 digits starting with 7. Example: 7911123456'
    },
    '+86': {
        'name': 'China',
        'length': [11],
        'regex': r'^1[3-9]\d{9}$',
        'error': 'Chinese mobile must be 11 digits starting with 13-19. Example: 13812345678'
    },
    '+49': {
        'name': 'Germany',
        'length': [10, 11],
        'regex': r'^1[567]\d{8,9}$',
        'error': 'German mobile must be 10-11 digits starting with 15, 16, or 17. Example: 15112345678'
    },
    '+33': {
        'name': 'France',
        'length': [9],
        'regex': r'^[67]\d{8}$',
        'error': 'French mobile must be 9 digits starting with 6 or 7. Example: 612345678'
    },
    '+81': {
        'name': 'Japan',
        'length': [10],
        'regex': r'^[789]0\d{8}$',
        'error': 'Japanese mobile must be 10 digits starting with 70, 80, or 90. Example: 9012345678'
    },
    '+7': {
        'name': 'Russia',
        'length': [10],
        'regex': r'^9\d{9}$',
        'error': 'Russian mobile must be 10 digits starting with 9. Example: 9123456789'
    },
    '+39': {
        'name': 'Italy',
        'length': [10],
        'regex': r'^3\d{9}$',
        'error': 'Italian mobile must be 10 digits starting with 3. Example: 3123456789'
    },
    '+34': {
        'name': 'Spain',
        'length': [9],
        'regex': r'^[67]\d{8}$',
        'error': 'Spanish mobile must be 9 digits starting with 6 or 7. Example: 612345678'
    }
}
...
def validate_email_domain(email):
    ...
    ONLY allows known email providers (Gmail, Yahoo, Outlook, etc.)
    ...
    # Step 1: Check for invalid characters (comma, spaces, etc.)
    if re.search(r'[,\s<>()[\]\\;:]', email):
        return False, "Email contains invalid characters"
    
    # Step 2: Basic format check - must have @ and proper structure
    basic_regex = r'^[a-zA-Z0-9._%+-]+@[a-zA-Z][a-zA-Z0-9.-]*\.[a-zA-Z]{2,}$'
    if not re.match(basic_regex, email):
        return False, "Invalid email format. Please use format: name@gmail.com"
    ...
    # Step 5: ONLY these domains are allowed (EXACT MATCH)
    valid_domains = [
        # Google
        'gmail.com',
        'googlemail.com',
    ...
    # Step 5.1: Check exact domain match
    if domain in valid_domains:
        return True, ""
    
    # Step 5.2: Allow educational/institutional domains (pattern-based)
    educational_patterns = ['.ac.in', '.edu', '.edu.in', '.ac.uk', '.edu.au']
    for pattern in educational_patterns:
        if domain.endswith(pattern):
            return True, ""
    
    # Check if it's a typo of common providers
    if 'gmail' in domain:
        return False, f"Invalid email domain '{domain}'. Did you mean gmail.com?"
    elif 'yahoo' in domain:
        return False, f"Invalid email domain '{domain}'. Did you mean yahoo.com?"
    elif 'hotmail' in domain:
        return False, f"Invalid email domain '{domain}'. Did you mean hotmail.com?"
    elif 'outlook' in domain:
        return False, f"Invalid email domain '{domain}'. Did you mean outlook.com?"
    else:
        return False, f"Email domain '{domain}' is not supported. Please use Gmail, Yahoo, Outlook, educational (.ac.in, .edu), or other major email providers."
# ==============================================================================
# STEP 2: ADD THIS VALIDATION FUNCTION (after PHONE_VALIDATION_RULES)
# ==============================================================================

def validate_phone_number(phone_number: str, country_code: str) -> Tuple[bool, str, str]:
    """
    Validate phone number based on country code
    
    Args:
        phone_number: Raw phone number (may contain spaces, dashes, etc.)
        country_code: Country code (e.g., '+91', '+1')
    
    Returns:
        Tuple of (is_valid: bool, error_message: str, sanitized_number: str)
    """
    # Return valid if phone is empty (optional field)
    ...
    # If no country code provided, use default validation
    if not country_code:
        if len(digits_only) < 8:
            return False, "Phone number must be at least 8 digits", digits_only
        if len(digits_only) > 15:
            return False, "Phone number must not exceed 15 digits", digits_only
        return True, "", digits_only
    
    # Get validation rules for the country
    rules = PHONE_VALIDATION_RULES.get(country_code)
    ...
    # Check length
    valid_lengths: List[int] = rules['length']
    
    if len(digits_only) < min(valid_lengths):
        return False, f"Phone number too short. {rules['error']}", digits_only
    
    if len(digits_only) > max(valid_lengths):
        return False, f"Phone number too long. {rules['error']}", digits_only
    
    # Check format with regex
    if not re.match(rules['regex'], digits_only):
        return False, f"Invalid phone format. {rules['error']}", digits_only
...
def send_OTP_email(email, OTP):
    ...
    1. Django send_mail() → uses EMAIL_BACKEND (SESWithFallbackBackend)
       ...
       → If SES fails → tries SMTP fallback (jaipalreddyk@lancieretech.com)
...
"""
    ...
    # This automatically tries SES first, then SMTP fallback
    # ============================================================
    try:
    ...
    try:
        ...
        smtp_host = fallback.get('HOST', 'smtp.office365.com')
...
def validate_email(request):
    """Validate email format and domain - called in real-time from frontend"""
    try:
        ...
        # Use your existing validate_email_domain function
        is_valid, error_message = validate_email_domain(email)
        ...
        with connection.cursor() as cursor:
            cursor.execute("SELECT COUNT(*) FROM tbl_Users WHERE email = %s AND Status = 1", [email])
...
def Register_User(request):
    ...
    is_valid_email, email_error = validate_email_domain(data['email'])
    if not is_valid_email:
        logger.error(f"Email validation failed: {email_error}")
        return JsonResponse({"Error": email_error}, status=400)

    # ✅ NEW: Phone number validation (add after email validation)
    phone_number = data.get('phone_number', '')
    country_code = data.get('country_code', '+91')  # Default to India
    
    if phone_number:  # Only validate if phone is provided
        is_valid_phone, phone_error, sanitized_phone = validate_phone_number(
            phone_number, 
            country_code
        )
        ...
        # Use sanitized phone number (digits only)
        data['phone_number'] = sanitized_phone
        logger.info(f"✓ Phone validated: {sanitized_phone} ({country_code})")
...
def Login_User(request):
    ...
    # ✅ GET: Optional MAC address from client
    mac_address = data.get('mac_address') or data.get('MAC_Address')
    
    # ✅ FIX: Get location data from client (matching frontend field names exactly)
    latitude = data.get('latitude')
    longitude = data.get('longitude')
    location_address = data.get('location_address') or data.get('location')
    location_accuracy = data.get('location_accuracy')
    location_source = data.get('location_source') or 'unknown'
    
    # ✅ ADD: Debug logging to verify location data received
    logging.info(f"📍 Login location data received:")
    logging.info(f"   - latitude: {latitude}")
    logging.info(f"   - longitude: {longitude}")
    logging.info(f"   - location_accuracy: {location_accuracy}")
    logging.info(f"   - location_source: {location_source}")
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if row and row[1] == password:
                ...
                # ✅ FIX: Record login history with ALL location data
                login_history_id = None
                if LOGIN_HISTORY_ENABLED:
                    logging.info(f"📍 Calling record_login_history with location: lat={latitude}, lng={longitude}")
                    login_history_id = record_login_history(
                        user_id=user_id,
                        request=request,
                    ...
                    logging.info(f"✓ Login history recorded: ID {login_history_id}")
...
#     try:
#         # ✅ FIX: Get user info BEFORE any session operations
#         user_id = request.session.get('User_Id')
#         session_id = request.session.session_key
        
#         logging.info(f"Logout attempt: User_ID={user_id}, Session_ID={session_id}")
        
#         if not user_id:
#             logging.warning("No user_id in session - user may already be logged out")
#             return JsonResponse({
#                 "Message": "No active session found",
#                 "Already_Logged_Out": True
#             }, status=SUCCESS_STATUS)
        
#         # ✅ FIX: Record logout BEFORE clearing session
#         logout_success = False
#         if LOGIN_HISTORY_ENABLED:
#             # Try with session_id first
#             logout_success = record_logout(user_id, session_id)
            
#             # ✅ FIX: If session_id didn't work, try without it
#             if not logout_success:
#                 logging.warning(f"Session-specific logout failed, trying user-only logout")
#                 logout_success = record_logout(user_id, None)
            
#             logging.info(f"✓ Logout recorded for user {user_id}: {logout_success}")
...
#             "User_Id": user_id,
#             "Logout_Recorded": logout_success
#         }, status=SUCCESS_STATUS)
...
def Logout_User(request):
    ...
    FIXED VERSION:
    - Reads User_Id from BOTH session AND request body
    - Properly calls record_logout to update Status to 'logged_out'
    - Has extensive logging for debugging
    """
    logging.info("")
    logging.info("=" * 70)
    logging.info("🚪 LOGOUT_USER ENDPOINT CALLED")
    logging.info("=" * 70)
    
    try:
        # =====================================================================
        # STEP 1: Get User_Id from multiple sources
        # =====================================================================
        ...
        logging.info(f"📌 From Session:")
        logging.info(f"   - User_Id: {session_user_id}")
        logging.info(f"   - Session_Key: {session_id}")
        
        # Source 2: Request Body (JSON)
        body_user_id = None
        request_body = {}
        ...
        try:
            if request.body:
                ...
                body_user_id = (
                    request_body.get('User_Id') or 
                    request_body.get('user_id') or 
                    request_body.get('userId') or
                    request_body.get('id') or
                    request_body.get('ID')
                )
                logging.info(f"📌 From Body - User_Id: {body_user_id}")
        except json.JSONDecodeError as e:
            logging.warning(f"⚠️ Could not parse JSON body: {e}")
        except Exception as e:
            logging.warning(f"⚠️ Error reading body: {e}")
        
        # =====================================================================
        # STEP 2: Determine final User_Id (prefer session, fallback to body)
        # =====================================================================
        
        user_id = session_user_id or body_user_id
        
        logging.info(f"")
        logging.info(f"🎯 FINAL User_Id to use: {user_id}")
        logging.info(f"   Source: {'session' if session_user_id else 'request body' if body_user_id else 'NONE'}")
        
        if not user_id:
            logging.warning("❌ No User_Id found in session or request body!")
            return JsonResponse({
                "Message": "No active session found",
                "Already_Logged_Out": True,
                "Logout_Recorded": False,
                "Debug": {
        ...
        try:
            user_id = int(user_id)
            logging.info(f"✓ User_Id converted to int: {user_id}")
        ...
        # =====================================================================
        # STEP 3: Record logout in database (THIS IS THE KEY STEP!)
        # =====================================================================
        ...
        if LOGIN_HISTORY_ENABLED:
            logging.info(f"")
            logging.info(f"📝 LOGIN_HISTORY_ENABLED = True")
            logging.info(f"📝 Calling record_logout(user_id={user_id}, session_id={session_id})")
            
            # First attempt: with session_id
            logout_recorded = record_logout(user_id, session_id)
            logging.info(f"📝 First attempt result: {logout_recorded}")
            
            # Second attempt: without session_id (if first failed)
            if not logout_recorded:
                logging.info(f"📝 First attempt failed, trying without session_id...")
                logout_recorded = record_logout(user_id, None)
                logging.info(f"📝 Second attempt result: {logout_recorded}")
        else:
            logging.warning("⚠️ LOGIN_HISTORY_ENABLED = False!")
            logging.warning("⚠️ Logout will NOT be recorded in database!")
        ...
        try:
            request.session.flush()
            logging.info("✓ Session flushed successfully")
        except Exception as e:
            logging.warning(f"⚠️ Could not flush session: {e}")
        ...
        response_data = {
            "Message": "Logout successful",
            "User_Id": user_id,
            "Logout_Recorded": logout_recorded
        }
        
        logging.info("")
        logging.info("=" * 70)
        if logout_recorded:
            logging.info("✅ LOGOUT SUCCESS - Status updated to 'logged_out' in database")
        else:
            logging.info("⚠️ LOGOUT PARTIAL - Session cleared but database NOT updated")
        ...
        logging.error(f"")
        logging.error(f"❌ EXCEPTION in Logout_User: {e}")
        import traceback
        logging.error(traceback.format_exc())
...
# ============================================================================
# CHANGE PASSWORD APIs - Add BEFORE urlpatterns
# ============================================================================
...
def Verify_Password(request):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            db_user_id, db_email, stored_password, full_name = row
            
            # Verify password
            if current_password != stored_password:
                logging.warning(f"Password verification failed for user {user_id}")
                return JsonResponse({
                    "Error": "Current password is incorrect",
                    "verified": False
                }, status=401)
            
            logging.info(f"Password verified successfully for user {user_id}")
        ...
        logging.error(f"Error in Verify_Password: {e}")
...
def Change_Password(request):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if current_password != stored_password:
                logging.warning(f"Wrong current password for user {user_id}")
...
def Update_User(request, id):
    ...
    # ✅ FIXED: Phone number validation with country-specific rules
    phone_number = data.get('phone_number', '')
    country_code = data.get('country_code', '+91')  # Default to India
    ...
    if phone_number:  # Only validate if phone is provided
        ...
        if not is_valid_phone:
            logging.error(f"Phone validation failed for user {id}: {phone_error}")
            return JsonResponse({"Error": phone_error}, status=BAD_REQUEST_STATUS)
    ...
    is_valid_email, email_error = validate_email_domain(data['email'])
    if not is_valid_email:
        logging.error(f"Email validation failed for user {id}: {email_error}")
        return JsonResponse({"Error": email_error}, status=BAD_REQUEST_STATUS)
...
def Face_Recognition_Login(request):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # ✅ ADD: Record login history for face recognition
            login_history_id = None
            if LOGIN_HISTORY_ENABLED:
                login_history_id = record_login_history(
                    user_id=user_id,
                    request=request,
                    login_method='face_recognition',  # ✅ Different method
            ...
            return JsonResponse({
                ...
                "Detection_Score": recognition_result['det_score'],
                "Session_Timeout": 86400,
                "Login_History_Id": login_history_id  # ✅ ADD
            }, status=200)
...
# ============================================================================
# ADD THIS API TO users7654.py (BEFORE urlpatterns)
# ============================================================================

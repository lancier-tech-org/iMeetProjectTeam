...
from attendance.mediapipe_pool import get_mediapipe_results
import logging
...
def release_face_model_gpu():
    """Release face model GPU memory after detection - calls Face Auth Service"""
    try:
        import torch
...
def stop_attendance_tracking_api(request):
    ...
    try:
        ...
        if success and is_last_participant:
            ...
            # ✅ NOW safe to release local GPU memory
            try:
                logger.info(f"🔄 Attempting to release local GPU memory...")
                ...
                if torch.cuda.is_available():
                    ...
                    gc.collect()
                    gpu_released = True
                    logger.info(f"✅ Local GPU cache cleanup completed for meeting {meeting_id}")
